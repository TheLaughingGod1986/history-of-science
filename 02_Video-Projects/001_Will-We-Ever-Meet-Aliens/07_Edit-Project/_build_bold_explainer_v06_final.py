#!/usr/bin/env python3
"""Build the final narration-led, no-repeat Bold Explainer v06 master."""
...
ORBIT_ROBOTICS = SFX / "aliens_orbit_robotics_v19.wav"
...
OUT = ROOT / "09_Final-Export/aliens_BOLD_EXPLAINER_v06_FINAL_POLISHED.mp4"
MANIFEST = ROOT / "07_Edit-Project/bold-explainer-v06-final-timeline.json"

VOICE_FILES = [
    VOICE / "aliens_vo_bold-v05_section-01_problem_ivc_kDch_v01.mp3",
    VOICE / "aliens_vo_bold-v05_section-02_explanations_ivc_kDch_v01.mp3",
    VOICE / "aliens_vo_bold-v05_section-03_search_ivc_kDch_v01.mp3",
    VOICE / "aliens_vo_bold-v05_section-04_solution-cliffhanger_ivc_kDch_v01.mp3",
]
...
def delayed_filter(label: str, times: list[float], volume: float) -> tuple[str, list[str]]:
    if not times:
        return "", []
    outputs = [f"{label}{index}" for index in range(len(times))]
    filters = [f"[{label}]asplit={len(times)}" + "".join(f"[{name}raw]" for name in outputs)]
    for name, timestamp in zip(outputs, times):
        delay = max(0, round(timestamp * 1000))
        filters.append(
            f"[{name}raw]volume={volume},adelay={delay}|{delay}[{name}]"
        )
    return ";".join(filters), outputs


def build_effects_mix(
    work: Path,
    duration: float,
    hook_split: float,
    ...
    effects = work / "effects-bed.wav"
    shimmer_times = [chapter_times[-1] + 7.0, max(0.0, duration - 34.0)]
    whoosh_filter, whoosh_outputs = delayed_filter("whoosh", chapter_times, 0.16)
    blip_filter, blip_outputs = delayed_filter("blip", orbit_times, 0.07)
    servo_filter, servo_outputs = delayed_filter(
        "servo", [time + 0.18 for time in orbit_times], 0.055
    )
    shimmer_filter, shimmer_outputs = delayed_filter("shimmer", shimmer_times, 0.11)
    brand_delay = max(0, round(hook_split * 1000))
    mix_outputs = [
        "space", "texture", "robotics", "brand",
        *whoosh_outputs, *blip_outputs, *servo_outputs, *shimmer_outputs,
    ]
    filters = [
        f"[0:a]atrim=duration={duration:.6f},volume=0.018[space]",
        f"[1:a]atrim=duration={duration:.6f},volume=0.012[texture]",
        f"[2:a]atrim=duration={duration:.6f},volume=0.008[robotics]",
        f"[3:a]volume=0.18,adelay={brand_delay}|{brand_delay}[brand]",
        "[4:a]anull[whoosh]",
        "[5:a]anull[blip]",
        "[6:a]anull[servo]",
        "[7:a]anull[shimmer]",
        ...
        servo_filter,
        shimmer_filter,
        "".join(f"[{name}]" for name in mix_outputs)
        + f"amix=inputs={len(mix_outputs)}:normalize=0,"
        + f"atrim=duration={duration:.6f}[fx]",
    ]
    run([
        "ffmpeg", "-y", "-hide_banner", "-loglevel", "error",
        "-stream_loop", "-1", "-i", str(SPACE_AMBIENCE),
        "-stream_loop", "-1", "-i", str(SFX_BED),
        "-stream_loop", "-1", "-i", str(ORBIT_ROBOTICS),
        "-i", str(BRAND_CHIME),
        "-i", str(CHAPTER_WHOOSH),
        "-i", str(ORBIT_BLIP),
        "-i", str(ORBIT_SERVO),
        "-i", str(SHIMMER),
        "-filter_complex", ";".join(item for item in filters if item),
...
def build_music_mix(
    ...
    effects = build_effects_mix(
        work, duration, hook_split, chapter_times, orbit_times
    )
    ...
    run([
        ...
        (
            ...
            "attack=12:release=350[duckedmusic];"
            "[2:a][0:a]sidechaincompress=threshold=0.020:ratio=6:"
            "attack=8:release=280[duckedfx];"
            "[0:a][duckedmusic][duckedfx]"
            "amix=inputs=3:weights='1 0.55 0.72':normalize=0,"
...
def main() -> None:
    required = [
        SCRIPT, BRAND, MUSIC_A, MUSIC_B, SFX_BED, SPACE_AMBIENCE,
        ORBIT_ROBOTICS, BRAND_CHIME, CHAPTER_WHOOSH, ORBIT_BLIP,
        ORBIT_SERVO, SHIMMER, *VOICE_FILES,
    ]
    ...
    with tempfile.TemporaryDirectory(prefix="orbit-bold-v05-") as temp:
        ...
        chapter_times = [
            section_durations[0] + BRAND_HOLD + SECTION_GAP,
            section_durations[0] + section_durations[1]
            + BRAND_HOLD + (2 * SECTION_GAP),
            section_durations[0] + section_durations[1] + section_durations[2]
            + BRAND_HOLD + (3 * SECTION_GAP),
        ]
        orbit_times: list[float] = []
        seen_poses: set[str] = set()
        for item in timeline:
            pose = item.get("orbit_pose")
            if (
                isinstance(pose, str)
                and pose != "brand-card"
                and pose not in seen_poses
            ):
                seen_poses.add(pose)
                orbit_times.append(float(item["start"]) + 0.45)
        final_audio = build_music_mix(
            work, voice_duration, hook_split, chapter_times, orbit_times
        )
        run([
            "ffmpeg", "-y", "-hide_banner", "-loglevel", "error",
    ...
    manifest = {
        "version": "bold-explainer-v06-final",
        "narration_led": True,
        ...
        "sound_effects": [
            str(SFX_BED), str(SPACE_AMBIENCE), str(ORBIT_ROBOTICS),
            str(BRAND_CHIME), str(CHAPTER_WHOOSH), str(ORBIT_BLIP),
            str(ORBIT_SERVO), str(SHIMMER),
        ],
        "output": str(OUT),
        "scene_count": 96,
