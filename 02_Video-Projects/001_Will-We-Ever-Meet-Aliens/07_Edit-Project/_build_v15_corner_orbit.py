#!/usr/bin/env python3
"""Rebuild picture with small solid corner Orbit narrator → v15.
...
OUT_PIC = ROOT / "09_Final-Export/aliens_broadcast_v15_corner_orbit_pic.mp4"
OUT = ROOT / "09_Final-Export/aliens_broadcast_v15_corner_orbit.mp4"
PROOF = ROOT / "09_Final-Export/aliens_v15_PROOF_corner_orbit_90s.mp4"
...
def proof_90s(data: dict) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="orbit_v15_proof_") as td:
        td = Path(td)
        bed, edl_shots = build_bed(shots, td)
        pic = td / "pic.mp4"
        print("compositing corner Orbit…")
...
def full_build(data: dict) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="orbit_v15_full_") as td:
        td = Path(td)
        ...
        print("compositing corner Orbit (full)…")
