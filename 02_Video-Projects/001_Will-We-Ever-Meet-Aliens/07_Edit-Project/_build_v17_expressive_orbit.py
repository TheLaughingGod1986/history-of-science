#!/usr/bin/env python3
"""Rebuild picture with over-the-top expressive corner Orbit → v17.

Reuses v10 EDL + v16 polished mix audio.
"""
...
MIX = ROOT / "07_Edit-Project/_mix_work_v16/final_mix.wav"
OUT_PIC = ROOT / "09_Final-Export/aliens_broadcast_v17_expressive_orbit_pic.mp4"
OUT = ROOT / "09_Final-Export/aliens_broadcast_v17_expressive_orbit.mp4"
PROOF = ROOT / "09_Final-Export/aliens_v17_PROOF_expressive_orbit_90s.mp4"
...
def proof_90s(data: dict) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="orbit_v17_proof_") as td:
        td = Path(td)
        bed, edl_shots = build_bed(shots, td)
        pic = td / "pic.mp4"
        print("compositing expressive Orbit…")
...
def full_build(data: dict) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="orbit_v17_full_") as td:
        td = Path(td)
        ...
        print("compositing expressive Orbit (full)…")
        wins = b._build_emotion_windows(markers, edl_shots)
        emotions = sorted({w["emotion"] for w in wins})
