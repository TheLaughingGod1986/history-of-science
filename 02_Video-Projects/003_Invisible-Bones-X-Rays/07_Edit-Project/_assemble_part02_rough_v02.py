#!/usr/bin/env python3
"""HOS 003 Part 02 rough v02 — same pipeline as v01, plate 08 remint only.

Ben FAIL 20 Sep 2026: 08_locks_the_door_v01 clips (bolt through strike plate).
v02 uses 08_locks_the_door_v02.mp4. Do not use clipped v01.
...
OUT = PROJ / "09_Final-Export/hos_003_part02_rough_v02.mp4"
ICLOUD = (
    Path.home()
    / "Library/Mobile Documents/com~apple~CloudDocs/HOS UAT"
    / "hos_003_part02_rough_v02.mp4"
)
WORK = PROJ / "07_Edit-Project/_part02_v02_work"
LABEL_DIR = PROJ / "07_Edit-Project/_part02_v02_labels"
META = PROJ / "07_Edit-Project/part02_rough_v02_land_meta.json"
XFADE = 0.35
CLIP_USE = 8.0
BED_VOL = 0.34
FPS = 24
...
# KEEP 02, 04–07, 08 v02, 09–11. No clipped 08 v01. No 03. No 01 chapter.
PLATES = [
...
FORBIDDEN_PLATES = ("08_locks_the_door_v01.mp4",)
...
def cue_from_align(
    ...
    fallback_labels = [
        (windows[0][0] + 0.6, windows[0][1] - 0.8, "COVERED TUBE"),
        (windows[1][0] + 0.5, windows[1][1] - 0.6, "CARDBOARD"),
        (windows[2][0] + 0.5, windows[2][1] - 0.6, "IT GLOWS"),
        (windows[3][0] + 0.5, windows[3][1] - 0.6, "FLUORESCENT"),
        (windows[4][0] + 0.5, windows[4][1] - 0.6, "X"),
        (windows[5][0] + 0.5, windows[5][1] - 0.6, "UNKNOWN"),
        (windows[7][0] + 0.5, windows[7][1] - 0.5, "A PATTERN"),
        (windows[8][0] + 0.4, min(windows[8][1] - 0.3, pic_dur - 0.15), "DETECTOR"),
    ]
    fallback_cards = [
        (
            windows[0][0] + 3.2,
            min(windows[0][1] - 0.2, windows[0][0] + 6.6),
            "WHY COVER THE TUBE",
            "Cathode rays were supposed to stay inside.",
        ),
        (
            windows[3][0] + 2.4,
            min(windows[3][1] - 0.2, windows[3][0] + 6.4),
            "FLUORESCENT SCREEN",
            "A screen painted to glow when light hits it.",
        ),
        (
            windows[4][0] + 2.8,
            min(windows[5][1] - 0.4, windows[4][0] + 7.2),
            "X FOR UNKNOWN",
            "He names the new ray X — unknown.",
        ),
        (
            windows[7][0] + 2.6,
            min(windows[7][1] - 0.2, windows[7][0] + 6.6),
            "PATTERN, NOT GHOST",
    ...
    for start, dur, name in raw:
        if start >= pic_dur - 0.4:
            snap = {
                "COVERED TUBE": windows[0],
                "CARDBOARD": windows[1],
                "FLUORESCENT": windows[3],
                "IT GLOWS": windows[2],
                "X": windows[4],
                "UNKNOWN": windows[5],
                "A PATTERN": windows[7],
...
def main() -> None:
    ...
    if any(name in FORBIDDEN_PLATES for name in PLATES):
        raise SystemExit("STOP: clipped 08_locks_the_door_v01 is forbidden in v02")
    if "08_locks_the_door_v02.mp4" not in PLATES:
        raise SystemExit("STOP: v02 assemble must use 08_locks_the_door_v02.mp4")
    ...
    if vo_dur - mix_dur > 0.4:
        print(
            ...
            f"{vo_dur - mix_dur:.1f}s overhang cut for v02 (no freeze-pad; "
            "03_cardboard_waiting still parked)",
            flush=True,
        )
    ...
    meta = {
        ...
        "bed_vol": BED_VOL,
        "plates": PLATES,
        "parked": ["01_chapter_cardboard_v01", "03_cardboard_waiting"],
        ...
        "house": "No Explorer. No Orbit. Flat-cardboard brief. Continuous 1x motion. 08 v02 only.",
    }
