#!/usr/bin/env python3
"""Assemble HOS 002 Part 04 rough v16 — LATE SHOTS SHARP remint.

Parent FAIL: hos_002_part04_rough_v15.mp4
  sha256 cd7a57b2979478a34ac7cc5fe5639aa156ae603b437674ccd724ab3fe994748c
Single blocker: LATE SHOTS SHARP — ghost doubles / left-half pixel mush ~110–125.

Remint ONLY: 10_family_before_weight, 11_publish_gaps, 11b_wait_and_hunt
KEEP from v15: Explorer 06, 09, 09b CLEAN LIGHT, written cards ~40, all earlier plates.
...
OUT = PROJ / "09_Final-Export/hos_002_part04_rough_v16.mp4"
...
def resolve_clip(pid: str) -> tuple[Path, str]:
    if pid in REMINT_V16_IDS:
        v16 = RAW_V16 / f"{pid}_v16.mp4"
        if v16.exists() and v16.stat().st_size >= 400_000:
            return v16, "v16_late_sharp_remint"
        raise SystemExit(f"missing reminted v16 plate {v16}")
...
def main() -> None:
    ...
    v15_sha = require_sha(KEEP_V15_PARENT, KEEP_V15_PARENT_SHA, "v15 parent")
    ...
    dur = probe(OUT)
    reminted = [pid for pid, src in zip(ordered_ids, sources) if src == "v16_late_sharp_remint"]
    ...
    print(
        ...
        f"  parent_v15_sha={v15_sha}\n",
        flush=True,
    )

    methods = {
        pid: "Flow Veo 3.1 Fast I2V late-sharp remint (no temporal-median deghost)"
        for pid in reminted
    }
    meta_path = PROJ / "07_Edit-Project/part04_rough_v16_land_meta.json"
        ...
        json.dumps(
            {
                ...
                "sha256": digest,
                "bible_main": "25bdefd",
                "parent_v15": KEEP_V15_PARENT.name,
                "parent_v15_sha256": v15_sha,
                "reminted_plates": reminted,
                "plate_sha256": {pid: plate_sha[pid] for pid in reminted},
                "plates": {
                    pid: {
                        "path": str((RAW_V16 / f"{pid}_v16.mp4").relative_to(PROJ)),
                        "sha256": plate_sha[pid],
                        "bytes": (RAW_V16 / f"{pid}_v16.mp4").stat().st_size,
                    }
                    for pid in reminted
                },
                "methods": methods,
                "kept_from_v15": sorted(KEEP_V15_IDS),
                "uat_blocker": "LATE SHOTS SHARP — ghost doubles / left-half pixel mush",
                "fail_times": {"family_ghost": 110, "publish_ghost": 121, "left_pixel": 125},
                "timeline": {
                ...
                "watch": "WATCH_part04_v16.txt",
                "doc": "PART04_V16_LATE_SHARP.md",
                "status": (
    ...
    watch = ICLOUD / "WATCH_part04_v16.txt"
    watch.write_text(
        "WATCH THIS FILE ONLY (Part 04 rough v16 — LATE SHOTS SHARP remint):\n"
        f"  {OUT.name}\n\n"
        "CoS / Picture→UAT gates for this cut:\n"
        "- Parent FAIL v15 sha cd7a57b2979478a34a… — single blocker LATE SHOTS SHARP.\n"
        "- Reminted ONLY 10 / 11 / 11b via clean Flow Veo (no temporal-median deghost).\n"
        "- KEEP from v15: Explorer 06, 09, 09b CLEAN LIGHT, written cards ~40.\n"
        "- Spot ~110 FAMILY FIRST: N-card sharp — NO ghost doubles.\n"
        "- Spot ~121–125 PUBLISH THE GAPS: finished sharp desk — NO full-scene ghosts, "
        "NO left-half pixel mush.\n"
    ...
    (PROJ / "07_Edit-Project/WATCH_part04_v16.txt").write_text(watch.read_text())
    (ICLOUD / "ZZ_OPEN_PART04_V16_ONLY.txt").write_text(
        "Part 04 current cut = hos_002_part04_rough_v16.mp4\n"
        "Part 01–03 LOCKED keepers.\n"
        "v15 FAIL (late ghost/pixel mush @ ~110–125) → v16 late-sharp remint — watch v16 only.\n"
    )
    (ICLOUD / "PART04_NEXT.txt").write_text(
        "Part 04 rough remint LANDED: hos_002_part04_rough_v16.mp4\n"
        "Watch WATCH_part04_v16.txt · PART04_V16_LATE_SHARP.md\n"
        "Scores → CoS. Do not declare PASS. Do not ping Ben.\n"
    )
