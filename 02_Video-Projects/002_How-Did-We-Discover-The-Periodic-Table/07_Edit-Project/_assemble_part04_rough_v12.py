...
OUT = PROJ / "09_Final-Export/hos_002_part04_rough_v12.mp4"
...
def resolve_clip(pid: str) -> tuple[Path, str]:
    if pid in REMINT_V12_IDS:
        v12 = RAW_V12 / f"{pid}_v12.mp4"
        if v12.exists() and v12.stat().st_size >= 400_000:
            return v12, "v12_clean_light_cards"
        raise SystemExit(f"missing reminted v12 plate {v12}")
...
def main() -> None:
    ...
    dur = probe(OUT)
    reminted = [pid for pid, src in zip(ordered_ids, sources) if src == "v12_clean_light_cards"]
    ...
    meta_path = PROJ / "07_Edit-Project/part04_rough_v12_land_meta.json"
    ...
    watch = ICLOUD / "WATCH_part04_v12.txt"
    watch.write_text(
        "WATCH THIS FILE ONLY (Part 04 rough v12 — Ben FAIL glasses/lamp/written cards):\n"
        f"  {OUT.name}\n\n"
    ...
    (PROJ / "07_Edit-Project/WATCH_part04_v12.txt").write_text(watch.read_text())
    (ICLOUD / "ZZ_OPEN_PART04_V12_ONLY.txt").write_text(
        "Part 04 current cut = hos_002_part04_rough_v12.mp4\n"
    ...
    (ICLOUD / "PART04_NEXT.txt").write_text(
        "Part 04 rough remint LANDED: hos_002_part04_rough_v12.mp4\n"
        "Watch WATCH_part04_v12.txt\n"
