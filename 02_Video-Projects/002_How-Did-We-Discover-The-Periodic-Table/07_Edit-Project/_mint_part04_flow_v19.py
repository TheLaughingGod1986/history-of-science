#!/usr/bin/env python3
"""Part 04 Flow remint v19 — UAT HARD FAIL bible 43d9405 (publish ghost + explorer).
...
QA = PROJ / "07_Edit-Project/_qa_part04_v19_flow"
META = PROJ / "07_Edit-Project/part04_mint_flow_v19_meta.json"
STARTS = PROJ / "04_Generated-Clips/part04/refs/v19_start_frames"
...
MAX_CREATES = int(os.environ.get("HOS_V19_MAX_CREATES", "3"))
...
PROMPTS = {
    "11_publish_gaps": (
        ...
        "sharp finished 3D cartoon desk — flat grid, coloured flasks, cards, magnifier, warm lamp. "
        "LOCKED CAMERA or tiny settle only — do NOT pan horizontally (horizontal pan causes ghost doubles). "
        "ZERO horizontal ghost on grid lines, flasks, lamp, magnifier in every frame. "
        "No Explorer. Continuous subtle object settle only. Silent. "
        + CLEAN + " " + SHARP + " " + REJECT + " " + STYLE
    ),
    "11b_wait_and_hunt": (
        "Image-to-video from the attached start frame. Wait-and-hunt beat: "
        "published flat grid with empty circular holes, flasks, soft lamp. "
        "LOCKED CAMERA or tiny settle only — no horizontal smear. "
        "ZERO ghost doubles on grid / flasks / lamp. No Explorer. Silent. "
        + CLEAN + " " + SHARP + " " + REJECT + " " + STYLE
    ),
    "06_explorer_leaves_gap": (
        "Image-to-video from the attached start frame. Explorer garnish beat: "
        "KEEP this exact back-view Explorer — finished wavy brown crown / bun, "
        "NO face-cloud blot, NO black hole dots in hair/scalp, gold glasses rim readable, "
        "teal coat, clean readable H/C/N/O cards (no garbled striping), clean warm lamp (no banding). "
        "LOCKED CAMERA or tiny settle. Continuous subtle motion only. Silent. "
        + CLEAN + " " + SHARP + " " + REJECT + " " + STYLE
    ),
}
...
def qa_clip(path: Path, tag: str) -> dict:
    ...
    start = STARTS / f"{tag.split('_try')[0]}_start_v19.jpg"
    mads = []
    if start.exists():
    ...
    if report["ghost_peak"] is not None and report["ghost_peak"] >= 0.52:
        report["reject"] = True
        report["reason"] = f"ghost_pair_frac {report['ghost_peak']:.3f} >= 0.52"
...
def mint_one(page, pid: str, meta: dict):
    start = STARTS / f"{pid}_start_v19.jpg"
    if not start.exists():
        raise SystemExit(f"missing {start}")
    dest = RAW / f"{pid}_v19.mp4"
    tmp = RAW / f"{pid}_v19.try.tmp.mp4"
    if dest.exists() and accept_mp4(dest) and os.environ.get("HOS_V19_SKIP_EXISTING", "1") == "1":
    ...
    for create_n in range(1, MAX_CREATES + 1):
        print(f"\n=== v19 I2V {pid} Create {create_n}/{MAX_CREATES} ===", flush=True)
        ...
        if report["reject"]:
            ...
            bad = REJECTED / f"{pid}_v19_try{create_n}_reject.mp4"
...
def main() -> None:
    ...
    if missing:
        raise SystemExit(
            ...
            "NO PAINT FALLBACK. Retry evidence in part04_mint_flow_v19_meta.json"
        )
