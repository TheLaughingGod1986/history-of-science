#!/usr/bin/env python3
"""Assemble HOS 002 Part 04 rough v23 — Explorer on-model + music through VO.

Parent: hos_002_part04_rough_v22.mp4
  sha256 78039206d044fd9ee61aed5dd3aa491fa697c32b2896ae7311a175db630b13d3

Ben UAT 10 Sep 2026 on v22 iCloud cut:
1) Explorer inconsistent with Germs / Part 01 / film lock (try3 OTS toy-scale).
2) Music dies ~mid-cut — bed file silent after ~85s and shorter than VO (~128s).

v23: swap ONLY 06_explorer_leaves_gap → try8 on-model KEEP; loop bed to cover VO.
Keep all other v22 plate shas. NO PAINT. Scores → CoS. Do not declare PASS.
"""
...
OUT = PROJ / "09_Final-Export/hos_002_part04_rough_v23.mp4"
...
def main() -> None:
    ...
    meta_path = PROJ / "07_Edit-Project/part04_rough_v23_land_meta.json"
        ...
        json.dumps(
            {
                ...
                "watch": "WATCH_part04_v23.txt",
                "ben_uat_v22": {
                    "explorer": "try3 OTS toy-scale superseded by try8B on-model",
                    "music": "bed silence after ~85s fixed via loop130 bed",
                },
                "status": (
                    "LANDED_FOR_UAT — Ben fixes: Explorer on-model + music through VO; "
                    "do not declare PASS"
                ),
            },
            indent=2,
    ...
    watch_body = (
        "WATCH THIS FILE ONLY (Part 04 rough v23 — Explorer on-model + music through VO):\n"
        f"  {OUT.name}\n\n"
        "Ben UAT fixes vs v22:\n"
        "- Explorer: try8B on-model (glasses/vest/satchel) — supersedes try3 OTS toy-scale.\n"
        "- Music: curious workshop bed looped to cover full VO (was silent after ~85s).\n\n"
        "CoS / Picture→UAT gates for this cut:\n"
        "- KEEP remints (v22 set + Explorer swap):\n"
        "  · 05_columns_families try2B Quality sha 49437e7e…e8caa\n"
        "  · 06_explorer_leaves_gap try8B sha fd4627d2…5c7a70 (on-model remint)\n"
        ...
        "- Scrub Explorer ~45–53s vs Germs/P01 lock. Scrub music present past 50s + through end.\n"
    ...
    watch = ICLOUD / "WATCH_part04_v23.txt"
    watch.write_text(watch_body)
    (PROJ / "07_Edit-Project/WATCH_part04_v23.txt").write_text(watch_body)
    (ICLOUD / "ZZ_OPEN_PART04_V23_ONLY.txt").write_text(
        "Part 04 current cut = hos_002_part04_rough_v23.mp4\n"
        "Part 01–03 LOCKED keepers.\n"
        "Ben fixes: on-model Explorer + music through VO — watch v23 only.\n"
    )
    (ICLOUD / "PART04_NEXT.txt").write_text(
        "Part 04 rough assemble LANDED: hos_002_part04_rough_v23.mp4\n"
        "Watch WATCH_part04_v23.txt\n"
        "Ben UAT: Explorer on-model + music continuous. Scores → CoS. Do not declare PASS.\n"
    )
