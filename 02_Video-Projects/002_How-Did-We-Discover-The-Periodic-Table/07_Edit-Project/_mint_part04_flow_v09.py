...
EXPLORER_GARNISH_LOCK = (
    ...
    "Explorer is a small accent, NEVER a medium portrait. Prefer PROFILE / three-quarter "
    "side so the FULL chestnut crown hair stays readable (messy waves on TOP of the "
    "head). Avoid a long pure-back shot that hides the face and flattens the crown. "
    "HARD REJECT face-to-camera teaching pose, waist-up hero framing, close-up face, "
    "or Explorer filling the middle of the frame."
)
...
PROMPTS = {
    "06_explorer_leaves_gap": (
        ...
        "CAMERA LOCK: stay on a PROFILE / three-quarter side view the ENTIRE 8 seconds "
        "so the full crown hair stays readable. He may lean and pin a card, then step "
        "slightly along the desk — but he must NOT turn his back to camera (no rear-of-head "
        "shot that can invent a bald crown). "
        ...
        "in the column, then steps slightly aside while still in profile. Continuous "
        "acting + gentle camera drift the whole clip — no Ken Burns still. Fully indoor "
        "wood panelled wall / filled bookcase behind the desk. "
