#!/usr/bin/env python3
"""UAT v07: Explorer into the workshop (not the Germs ward) + liquid-only 07/11."""
...
REJECT = PROJ / "04_Generated-Clips/part01/_rejected_uat_v07"
START = PROJ / "09_Final-Export/_uat_part01_v06/12_workshop_hold_t1.jpg"
META = PROJ / "07_Edit-Project/part01_remint_uat_v07_meta.json"
MODEL = "Veo 3.1 - Fast"
...
GLASS = (
    ...
    "FORBIDDEN inside glass: powder, cotton, granular solids, moss, crystal chunks, "
    "gold nuggets, beads, marbles, glowing energy orbs."
)
JAR_IDS = ("07_shelf_names_grow", "11_chemists_more_names")


def archive(src: Path, name: str) -> None:
    if not src.exists():
        return
...
def main() -> None:
    ...
    meta: dict = {"engine": "flow-ui", "model": MODEL, "mode": "uat-v07", "plates": []}
    RAW.mkdir(parents=True, exist_ok=True)

    from playwright.sync_api import sync_playwright

    profile = flow.profile_path(PROFILE)
    print(f"Flow UAT v07 profile={profile}", flush=True)
    with sync_playwright() as p:
        ...
        try:
            ...
            archive(dest, "05_explorer_ore_gas_v01_ward.mp4")
            print(f"\n=== I2V Explorer into workshop {pid} ===", flush=True)
            ...
            for pid in JAR_IDS:
                dest = RAW / f"{pid}_v01.mp4"
                archive(dest, f"{pid}_v01.mp4")
                prompt = f"{STYLE} {GLASS} {plates[pid]['prompt']}"
                print(f"\n=== T2V liquid-only {pid} ===", flush=True)
                info = flow.generate_clip(
                    page,
                    prompt,
                    dest,
            ...
            ctx.close()
    print("OK UAT v07 remint finished", flush=True)
