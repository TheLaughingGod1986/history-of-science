#!/usr/bin/env python3
"""Dedicated 09b_risk_hold try10B submit on live Mini CDP — nocup cropL931, lamp OFF-FRAME (bible 8ddbf99)."""
...
LOCK = PROJ / "04_Generated-Clips/part04/refs/v20_start_frames/09b_risk_hold_start_v20_try10B_nocup_opaque_glow.jpg"
KEEP_TRY8 = PROJ / "04_Generated-Clips/part04/refs/v20_start_frames/09b_risk_hold_start_v20_try8_KEEP_cf182dc.jpg"
TRY10A = PROJ / "04_Generated-Clips/part04/refs/v20_start_frames/09b_risk_hold_start_v20_try10_nocup_sidelit.jpg"
FORBIDDEN_FLAT = "2a651abc673e719af8f6647f264da28a45d41950473489193c72f9a0c4481182"
OUT = PROJ / "07_Edit-Project/_qa_part04_v20_flow/submit_09b_risk_hold_try10B.json"
...
def main() -> None:
    ...
    if not LOCK.exists():
        raise SystemExit(f"ABORT: missing try10B lock {LOCK}")
    s = sha(START)
    if s == FORBIDDEN_FLAT:
    ...
    if TRY10A.exists() and s == sha(TRY10A):
        raise SystemExit("ABORT: try10A DNA (cup returned) — use try10B cropL931")
    if s != sha(LOCK):
        raise SystemExit("ABORT: active start != try10B lock")
    for ban in ("molten bead", "shade cup", "OFF-FRAME", "CARDLESS", "8ddbf99"):
    ...
    with sync_playwright() as p:
        ...
        tmp = Path("/tmp/hos_v20_09b_try10B_stub.mp4")
        if tmp.exists():
            tmp.unlink()
        info: dict = {}
        try:
        ...
        out = {
            ...
            "start_sha256": s,
            "start_dna": (
                "try10B nocup opaque-glow: try9C cardless moon empty-chair cropL931 — "
                "lamp FULLY OFF FRAME; warm off-screen side glow; bible 8ddbf99; "
                "NOT try10A cropL791 (cup returned); NOT try8 KEEP; NOT flat; paint banned"
            ),
            "model": MODEL,
