#!/usr/bin/env python3
"""Dedicated 09b_risk_hold try11 submit on live Mini CDP.

DNA: ZERO desk-lamp/gooseneck/shade/bulb/fixture; CLEAN LIGHT=moon+window spill+soft ambient ONLY.
...
LOCK = REF / "09b_risk_hold_start_v20_try11_zerolamp_moonwin.jpg"
...
OUT = PROJ / "07_Edit-Project/_qa_part04_v20_flow/submit_09b_risk_hold_try11.json"
...
def main() -> None:
    ...
    if not LOCK.exists():
        raise SystemExit(f"ABORT: missing try11 lock {LOCK}")
    s = sha(START)
    if s == FORBIDDEN_FLAT:
    ...
    if TRY10D.exists() and s == sha(TRY10D):
        raise SystemExit("ABORT: active start still try10D — need try11 lock")
    if s != sha(LOCK):
        raise SystemExit("ABORT: active start != try11 lock")
    # try11 must NOT equal try10D cropL931 — that is intentional (same nocup DNA)
    for need in (
        "molten bead",
        "shade cup",
        "CARDLESS",
    ...
    with sync_playwright() as p:
        ...
        tmp = Path("/tmp/hos_v20_09b_try11_stub.mp4")
        if tmp.exists():
            tmp.unlink()
        info: dict = {}
        try:
        ...
        out = {
            ...
            "start_sha256": s,
            "start_dna": (
                "try11 zerolamp moonwin: try9C cardless empty-chair moon KEEP cropL18+top03 — "
                "ZERO desk-lamp/gooseneck/shade/bulb/fixture; CLEAN LIGHT=moon+window spill+soft ambient ONLY; "
                "LOCKED FRAMING; bible 8ddbf99; Soft Empty Chairs OK; finished 3D; NOT try10A–D; paint banned"
            ),
            ...
            "parent_try10D": "SELF_REJECT — invents cup+bead late despite nolamp tightL + locked framing",
            "paint": "banned",
            "assemble": False,
        }
