#!/usr/bin/env python3
"""Auth+credits gate for Part 03 v09 — STOP before mint if wrong account."""
...
OUT = Path(__file__).resolve().parent / "logs" / "flow_auth_probe_part03_v09.json"
PROFILE = Path(
    os.environ.get(
        "ORBIT_FLOW_PROFILE",
...
def main() -> None:
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            for marker in DIE:
                if marker in body:
                    result["error"] = f"STOP BLOCKED: credits death ({marker})"
                    OUT.parent.mkdir(parents=True, exist_ok=True)
                    OUT.write_text(json.dumps(result, indent=2))
                    raise SystemExit(result["error"])
            if not active or active not in ALLOWED or "benoats86" in (active or ""):
                result["error"] = f"BLOCKED_AUTH: active={active}"
                OUT.parent.mkdir(parents=True, exist_ok=True)
                OUT.write_text(json.dumps(result, indent=2))
                raise SystemExit(result["error"])
            result["ok"] = True
