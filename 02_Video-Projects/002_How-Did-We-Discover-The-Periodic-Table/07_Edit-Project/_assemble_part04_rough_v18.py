#!/usr/bin/env python3
"""Assemble HOS 002 Part 04 rough v18 — Ben OVERRIDE FAIL remint.

Parent FAIL: hos_002_part04_rough_v17.mp4 (Ben OVERRIDE)
  sha256 e4cc41d8cf050f6726e6b3d5cbbbff10285656c588f3a758b4a29e1ec1f998fd
Ben FAIL: publish ghosts · explorer face · family lamp/panel.

Remint: 06_explorer_leaves_gap · 10_family_before_weight · 11_publish_gaps · 11b_wait_and_hunt
KEEP: written cards ~40 · CLEAN LIGHT 09/09b.
...
OUT = PROJ / "09_Final-Export/hos_002_part04_rough_v18.mp4"
...
KEEP_V16_IDS = set()  # plate 10 reminted in v18
KEEP_V15_IDS = {
    "09_risk_bet",
...
def main() -> None:
    ...
    dur = probe(OUT)
    reminted = [pid for pid, src in zip(ordered_ids, sources) if src == "v18_ben_override_remint"]
    plate06 = RAW_V18 / "06_explorer_leaves_gap_v18.mp4"
    ...
    methods = {
        pid: "painted single-exposure v18 (Flow harvest/agent fail; Gemini prepaid 429; NOT temporal-median)"
        for pid in reminted
    }
    meta_path = PROJ / "07_Edit-Project/part04_rough_v18_land_meta.json"
        ...
        json.dumps(
            {
                ...
                "plates": {
                    pid: {
                        "path": str((RAW_V18 / f"{pid}_v18.mp4").relative_to(PROJ)),
                        "sha256": plate_sha[pid],
                        "bytes": (RAW_V18 / f"{pid}_v18.mp4").stat().st_size,
                    }
                    for pid in reminted
                },
                "methods": methods,
                "kept_from_v16": sorted(KEEP_V16_IDS),
                "kept_from_v15_partial": sorted(KEEP_V15_IDS),
                "kept_from_v15": sorted(KEEP_V15_IDS),
                "uat_blocker": "BEN OVERRIDE FAIL — publish ghosts · explorer face · family lamp/panel",
                ...
                "doc": "PART04_V18_BEN_OVERRIDE.md",
                "status": (
    ...
    watch = ICLOUD / "WATCH_part04_v18.txt"
    watch.write_text(
        "WATCH THIS FILE ONLY (Part 04 rough v18 — Ben OVERRIDE FAIL remint):\n"
        f"  {OUT.name}\n\n"
        "CoS / Picture→UAT gates for this cut:\n"
        "- Parent FAIL v17 sha e4cc41d8… — Ben OVERRIDE with stills (publish ghosts · "
        "explorer face · family lamp/panel).\n"
        "- Reminted 06 / 10 / 11 / 11b via single-exposure painted continuous motion.\n"
        "- KEEP: 09/09b CLEAN LIGHT, written cards ~40.\n"
        "- Spot ~47–53 Explorer: finished crown, NO face-cloud blot, NO black hole dots, "
        "gold glasses, clean cards.\n"
        "- Spot ~110 FAMILY FIRST: H/C/N/O sharp, clean lamp, ONE soft Empty Chairs panel.\n"
        "- Spot ~121 / ~125 PUBLISH THE GAPS: one solid lamp, one solid flask set, one solid "
        "grid — NO horizontal ghost doubles.\n"
        "- Spot ~102–105 CLEAN LIGHT KEEP: soft panel, no lamp lava.\n"
        "- Credit note: Gemini prepaid 429; Flow create stalled (Agent settings / harvest).\n"
    ...
    (PROJ / "07_Edit-Project/WATCH_part04_v18.txt").write_text(watch.read_text())
    (ICLOUD / "ZZ_OPEN_PART04_V18_ONLY.txt").write_text(
        "Part 04 current cut = hos_002_part04_rough_v18.mp4\n"
        "Part 01–03 LOCKED keepers.\n"
        "v17 Ben OVERRIDE FAIL → v18 remint — watch v18 only.\n"
    )
    (ICLOUD / "PART04_NEXT.txt").write_text(
        "Part 04 rough remint LANDED: hos_002_part04_rough_v18.mp4\n"
        "Watch WATCH_part04_v18.txt · PART04_V18_BEN_OVERRIDE.md\n"
