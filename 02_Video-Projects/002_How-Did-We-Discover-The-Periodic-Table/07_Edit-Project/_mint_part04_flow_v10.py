#!/usr/bin/env python3
"""Part 04 Flow Veo 3.1 Fast remint v10 — Explorer PROFILE/BACK pose lock (colour DNA keep).

CoS UAT FAIL v09: Explorer face-on / ¾ hero ~45–53s on plate 06.
Remint ONLY 06_explorer_leaves_gap → rough_v10.

KEEP: v09 timeline everything else (02b/05/09/09b); colour DNA; P01–P03 untouched.
...
RAW = PROJ / "04_Generated-Clips/part04/raw/v10_fast"
REJECTED = RAW / "_rejected"
QA_STILLS = PROJ / "07_Edit-Project/_qa_part04_v10_plate"
META = PROJ / "07_Edit-Project/part04_mint_flow_v10_meta.json"
...
EDIT_DL = Path(__file__).resolve().parent / "_harvest_part04_v10_edit_download.py"
...
MAX_CREATES = int(os.environ.get("HOS_V10_MAX_CREATES", "2"))
CREATE_OFFSET = int(os.environ.get("HOS_V10_CREATE_OFFSET", "0"))

START_FRAME_06 = PROJ / (
    "04_Generated-Clips/part04/refs/v10_start_frames/06_explorer_leaves_gap_start_v10.jpg"
)
# Prefer PROFILE / back start (never face-on fail stills)
START_FRAME_PREF = PROJ / (
    "04_Generated-Clips/part04/refs/v10_start_frames/_ref_profile_from_v09_t4.jpg"
)
DNA_GERMS = PROJ / "04_Generated-Clips/part04/refs/v10_dna/germs_lock.jpg"
DNA_P03 = PROJ / "04_Generated-Clips/part04/refs/v10_dna/p03_keep.jpg"
DNA_SHEET = PROJ / "04_Generated-Clips/part04/refs/v10_dna/character_sheet.jpg"
DNA_OK_PROFILE = PROJ / "04_Generated-Clips/part04/refs/v10_dna/ok_profile_49.jpg"

PARENT_V09 = PROJ / "09_Final-Export/hos_002_part04_rough_v09.mp4"
PARENT_V09_SHA = "2981b26ae720edc3a8f5456a99fb5a3ac727bc79b850dabe5ad9cc57cf61eb61"
...
REJECT = (
    ...
    "academic blazer instead of teal trenchcoat; teaching/presenting-to-camera pose; face-on Explorer; ¾-front hero; hands-on-hips present-to-viewer; mid-clip flip from profile to face-on."
)
...
PROMPTS = {
    "06_explorer_leaves_gap": (
        ...
        "covering the entire crown (tufts on TOP), fair warm-tan skin, dark teal coat. "
        f"{HAT_LOCK} {COLOUR_DNA_LOCK} {POSE_LOCK} "
        ...
        "Toy-scale garnish on desk. Full crown hair stays readable. "
        "He stays a small teal figurine on the honey-wood chemist desk. Beat: in PROFILE "
        "or from behind he pins one cream card into a vertical card column, leaves a "
        "glowing vacant seat/gap in the column, then steps slightly aside — still profile "
        "or back, never turning to the lens. Continuous acting + gentle camera drift the "
        "whole clip — no Ken Burns still. Fully indoor wood panelled wall / filled "
        "bookcase behind the desk (no window/sky/roofs/blue fills). "
        f"{EXPLORER_GARNISH_LOCK} {WINDOW_LOCK} {DESK_PROP_LOCK} "
        "Silent. HARD REJECT: face-on, ¾ hero, hands-on-hips present, hat, bald crown, "
        "darkened skin, wrong coat hue, face-hero, giant Explorer, twins, window, "
        "exterior sky, navy sky boxes, roofs, model-town. "
        + REJECT + " " + STYLE
    ),
}
...
def dest_for(plate_id: str) -> Path:
    return RAW / f"{plate_id}_v10.mp4"


def resolve_start_frame() -> Path:
    """Hat-free on-colour start. Prefer latest composed v10 start (sheet full crown)."""
...
def main() -> None:
    ...
    if not _truthy("HOS_FLOW_T2V_ONLY") and not _truthy("HOS_FLOW_I2V_FIRST"):
        ...
        print("  v10 default: I2V hat-free colour DNA start", flush=True)
    ...
    if bad:
        raise SystemExit(f"STOP: refuse remint via v10: {sorted(bad)}")
    ...
    meta: dict = {
        ...
        "raw": str(RAW),
        "continuity": "v10_explorer_profile_pose_colour_dna",
        "parent_v09_sha": PARENT_V09_SHA,
        "start_frame_06": str(START_FRAME_06),
        "dna_refs": [str(DNA_SHEET), str(DNA_GERMS), str(DNA_P03)],
        "locks": ["NO_HAT", "fair_warm_skin", "dark_teal_coat", "toy_scale_garnish", "profile_or_back_pose"],
    ...
    print(f"  HAT_LOCK + COLOUR_DNA_LOCK + POSE_LOCK armed", flush=True)
