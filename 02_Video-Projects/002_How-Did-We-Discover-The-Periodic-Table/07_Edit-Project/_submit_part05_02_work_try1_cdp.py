#!/usr/bin/env python3
"""Part 05 plate 02 — guests as work BOARDTOP (T2V Quality). Same cycle as 01 KEEP.

Uses the same start_counter (max 2). 01 KEEP: wall-chart chapter, not moon-desk.
"""
...
PROMPT_02 = (
    "TEXT-TO-VIDEO. Plate 02_guests_as_work try1 — they arrive as work. "
    ...
    "HERO ACTION (must be readable muted): BOARDTOP close of a honey wood desk. "
    ...
    "Continuous settle the whole 8 seconds. Daytime window. NO moon. "
...
JOBS = [
    {
        ...
        "which": "boardtop_work",
        "prompt": PROMPT_02,
        "out": QA_DIR / "submit_02_guests_as_work_try1.json",
    },
]
...
def bump_start_counter(plate: str) -> int:
    ...
    START_COUNTER.write_text(
        json.dumps(
            {
                "cycle": "v01_plate01_keep_then_02",
                "plate": plate,
                "starts": n,
...
def submit_one(page, job: dict) -> dict:
    ...
    out = {
        ...
        "paint": "banned",
        "assemble": False,
        "note": "P05 plate-first: 01 KEEP wall-chart; 02 BOARDTOP work",
    }
...
def main() -> None:
    with sync_playwright() as p:
        ...
        for job in JOBS:
            ...
            time.sleep(2)
        print(json.dumps({"submitted": urls, "next": "harvest 02 → plate UAT"}, indent=2), flush=True)
