#!/usr/bin/env python3
"""ONE-SHOT remint Part 01 plate 10 — Flow Veo I2V (T2V fallback).

UAT FAIL v13: blue rectangular scrub/mask around ore (~69–76s).
Also keep colourless shimmer under grate (NO orange/fire/embers).
KEEP: ore IN brass pan · no flask beside scale · continuous motion · no Orbit.
"""
...
SCENE = (
    ...
    "around the ore or grate — clean natural pixels only. "
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if "out of google flow credits" in body or "you're out of google flow credits" in body:
                shot = REJECT / "10_rock_not_fire_v14_flow_no_credits.png"
                page.screenshot(path=str(shot), full_page=True)
                raise SystemExit(
                    f"STOP: Flow UI still shows out-of-credits after Ben top-up. screenshot={shot}"
                )
            ...
            info: dict
            print("\n=== I2V 10_rock_not_fire (v14 colourless · no blue mask) ===", flush=True)
            try:
                ...
                # If credits banner appeared mid-run, stop hard (no local scrub)
                body2 = (page.locator("body").inner_text(timeout=5000) or "").lower()
                if "out of google flow credits" in body2:
                    shot = REJECT / "10_rock_not_fire_v14_flow_no_credits_mid.png"
                    page.screenshot(path=str(shot), full_page=True)
                    raise SystemExit(
                        f"STOP: Flow out-of-credits during I2V. screenshot={shot} err={i2v_err}"
                    )
                print("\n=== FALLBACK T2V scenery (colourless · no blue mask) ===", flush=True)
                if tmp.exists():
                    tmp.unlink()
                info = flow.generate_clip(
                    page,
            ...
            ctx.close()
    print("OK plate10 v14 Flow remint finished", flush=True)
