#!/usr/bin/env python3
"""Assemble HOS 002 Part 04 rough v05 — scrub brown panel ×2.

Parent: hos_002_part04_rough_v05.mp4 sha 79dd7650…
...
RAW_V05 = PROJ / "04_Generated-Clips/part04/raw/v05_fast"
RAW_V04 = PROJ / "04_Generated-Clips/part04/raw/v04_fast"
LABEL_DIR = PROJ / "04_Generated-Clips/part04/refs/v05_side_labels"
VO = PROJ / "02_Voiceover/part04_empty_chairs_v01.wav"
BED = PROJ / "05_Music/hos_002_part01_curious_workshop_v02_norm.wav"
OUT = PROJ / "09_Final-Export/hos_002_part04_rough_v05.mp4"
KEEP_V04 = PROJ / "09_Final-Export/hos_002_part04_rough_v04.mp4"
KEEP_V04_SHA = "fda60b04bd3785b1f01b61c5c76b786a841d21152df29ed398ce5b5e4490554d"
...
REMINT_V05_IDS = {"02b_cards_sixty_three"}
KEEP_V02_IDS = {"05_columns_families", "06_explorer_leaves_gap"}
KEEP_V04_IDS = {"09_risk_bet", "09b_risk_hold"}
...
def main() -> None:
    ...
    if not KEEP_V04.exists():
        raise SystemExit(f"missing parent v03 {KEEP_V04}")
    v03_sha = sha256(KEEP_V04)
    if v03_sha != KEEP_V04_SHA:
        raise SystemExit(
            f"STOP: v04 parent sha mismatch want {KEEP_V04_SHA} got {v03_sha}"
        )
    print(f"KEEP v04 parent sha OK {v03_sha}", flush=True)
    ...
    dur = probe(OUT)
    reminted = [pid for pid, src in zip(ordered_ids, sources) if src == "v05_veo"]
    print(
        f"LANDED {OUT}\n"
        f"  path={OUT}\n"
        f"  bytes={bytes_n}\n"
        ...
        f"  parent_v04_sha={v03_sha}\n",
        flush=True,
    )

    meta_path = PROJ / "07_Edit-Project/part04_rough_v05_land_meta.json"
    ...
    watch = ICLOUD / "WATCH_part04_v05.txt"
    watch.write_text(
        "WATCH THIS FILE ONLY (Part 04 rough v05 — scrub brown panel ×2):\n"
        f"  {OUT.name}\n\n"
    ...
    (ICLOUD / "ZZ_OPEN_PART04_V04_ONLY.txt").write_text(
        "Part 04 current cut = hos_002_part04_rough_v05.mp4\n"
    ...
    (ICLOUD / "PART04_NEXT.txt").write_text(
        "Part 04 rough remint LANDED: hos_002_part04_rough_v05.mp4\n"
        "Watch WATCH_part04_v05.txt\n"
