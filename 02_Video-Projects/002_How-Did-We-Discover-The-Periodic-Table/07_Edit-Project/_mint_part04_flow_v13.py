"""Part 04 Flow Veo 3.1 Fast remint v13 — CLEAN LIGHT + WRITTEN CARDS (chair fire / lamp smoke / blank cards).

Parent FAIL: hos_002_part04_rough_v12.mp4 sha 175c40a948a24899507266d3f4bccf39f9f51a2906441cae4557bcbd312fe6c2
HARD blockers: (1) leather chair engulfed in WHITE FLAMES / seat fire cluster
(2) lamp smoke wisps (3) blank stack tops + blank foreground hero cards.

Remint ONLY: 04_sort_atomic_weight, 05_columns_families, 09_risk_bet, 09b_risk_hold.
KEEP: 06_explorer_leaves_gap CLEARED; 05b/07/08/08b/10 v12 cleared takes; 02b; 07b; P01–P03 frozen.
Empty Chairs glow = soft rectangular backrest / vacant-seat light ONLY — NEVER flames, NEVER white fire in seat, NEVER smoke from lamp.
Flow: benoats@googlemail.com on /u/1/. Scores → CoS. Do not declare PASS. Do not ping Ben.
"""
...
QA_STILLS = PROJ / "07_Edit-Project/_qa_part04_v13_plate"
META = PROJ / "07_Edit-Project/part04_mint_flow_v13_meta.json"
...
MAX_CREATES = int(os.environ.get("HOS_V13_MAX_CREATES", "2"))
CREATE_OFFSET = int(os.environ.get("HOS_V13_CREATE_OFFSET", "0"))

START_FRAMES = {
    "04_sort_atomic_weight": PROJ / "04_Generated-Clips/part04/refs/v13_start_frames/04_sort_atomic_weight_start_v13.jpg",
    "05_columns_families": PROJ / "04_Generated-Clips/part04/refs/v13_start_frames/05_columns_families_start_v13.jpg",
    "09_risk_bet": PROJ / "04_Generated-Clips/part04/refs/v13_start_frames/09_risk_bet_start_v13.jpg",
    "09b_risk_hold": PROJ / "04_Generated-Clips/part04/refs/v13_start_frames/09b_risk_hold_start_v13.jpg",
}
START_FRAME_PREF = START_FRAMES["05_columns_families"]
...
PARENT_V12 = PROJ / "09_Final-Export/hos_002_part04_rough_v12.mp4"
PARENT_V12_SHA = "175c40a948a24899507266d3f4bccf39f9f51a2906441cae4557bcbd312fe6c2"
...
LAMP_CLEAN_LOCK = (
    ...
    "leather chair on fire, flames in leather cushions, WHITE FIRE CLUSTER in the seat, "
    "white flame shapes rising from the chair, smoke wisps from lamp or chair, "
    "any vapor/smoke trail near the lamp head. "
    "ZERO open flames anywhere. Empty Chairs glow = soft rectangular backrest glow / "
    "vacant-seat light ONLY — like a soft softbox rectangle on the empty chair — "
    "NEVER flames, NEVER white fire cluster in the seat, NEVER smoke from lamp."
)

WRITTEN_CARDS_LOCK = (
    "WRITTEN CARDS LOCK: every visible element/period card face AND every stack TOP shows "
...
PROMPTS = {
    "04_sort_atomic_weight": (
        ...
        "WRITTEN cream element cards — every card face and every stack TOP shows readable "
        "ink H/C/O/N/Li marks with small numbers — NEVER blank tops, NEVER blank flat cards. "
        "Stack edges look like paper layers with marks on tops. Cards slide and sort by "
        "atomic weight across the honey wood. Soft warm desk-lamp glow ONLY — no fire spit, "
        "sparks, candle flames, or smoke. Continuous sort motion. Fully indoor wood/bookcase "
        "— NO moon window, NO flat blue sky. "
    ...
    "05_columns_families": (
        ...
        "WRITTEN cream element cards — every stack TOP and every upright/flat card shows "
        "readable H/C/O/N/Li/Be ink — NEVER blank stack tops, NEVER blank outer tops, "
        "NEVER blank flat cards. Paper-layer stack edges with marks on tops. Cards settle "
        "into family columns. Soft warm desk-lamp glow ONLY — no sparks under bulb, no "
        "candle flames, no smoke. Continuous settle motion. Fully indoor wood/bookcase. "
        f"{LAMP_CLEAN_LOCK} {WRITTEN_CARDS_LOCK} {WINDOW_LOCK} {DESK_PROP_LOCK} "
        "Silent. No people. No Explorer. "
        + REJECT + " " + STYLE
    ),
...
def dest_for(plate_id: str) -> Path:
    return RAW / f"{plate_id}_v13.mp4"
