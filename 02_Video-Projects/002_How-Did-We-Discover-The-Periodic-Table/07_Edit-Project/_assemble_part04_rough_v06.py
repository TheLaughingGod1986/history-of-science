...
OUT = PROJ / "09_Final-Export/hos_002_part04_rough_v06.mp4"
KEEP_V05 = PROJ / "09_Final-Export/hos_002_part04_rough_v05.mp4"
KEEP_V05_SHA = "a7c9741c32f9689c019d5c8695b36a8e8d2bbbebfb19b93ee53b6d584b8a43fc"
...
REMINT_V06_IDS = {"02b_cards_sixty_three"}
...
def main() -> None:
    ...
    if not KEEP_V05.exists():
        raise SystemExit(f"missing parent v05 {KEEP_V05}")
    v05_sha = sha256(KEEP_V05)
    if v05_sha != KEEP_V05_SHA:
        raise SystemExit(
            f"STOP: v05 parent sha mismatch want {KEEP_V05_SHA} got {v05_sha}"
        )
    print(f"KEEP v05 parent sha OK {v05_sha}", flush=True)
    ...
    dur = probe(OUT)
    reminted = [pid for pid, src in zip(ordered_ids, sources) if src == "v06_veo"]
    print(
        f"LANDED {OUT}\n"
        f"  path={OUT}\n"
        f"  bytes={bytes_n}\n"
        ...
        f"  parent_v05_sha={v05_sha}\n",
        flush=True,
    )

    meta_path = PROJ / "07_Edit-Project/part04_rough_v06_land_meta.json"
    ...
    (PROJ / "07_Edit-Project/WATCH_part04_v06.txt").write_text(watch.read_text())
