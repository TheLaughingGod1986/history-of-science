#!/usr/bin/env python3
"""Assemble HOS 002 Part 03 rough v02 — hall-continuity remint.

CoS gate: plate-01 hall DNA continuity, real Veo motion, Animistry side labels,
Explorer once ~28–36s (plate 05), no Ken Burns.
...
LABEL_DIR = PROJ / "04_Generated-Clips/part03/refs/v02_side_labels"
VO = PROJ / "02_Voiceover/part03_ruler_for_atoms_v01.wav"
BED = PROJ / "05_Music/hos_002_part01_curious_workshop_v02_norm.wav"
OUT = PROJ / "09_Final-Export/hos_002_part03_rough_v02.mp4"
...
# KEEP from v01 PASS notes: A RULER FOR ATOMS / FIX THE WEIGHTS / CHAOS HAS ADDRESS + beat labels
SIDE_LABELS = [
...
def main() -> None:
    ...
    # Supersede any prior part03 roughs (keep v02 as current)
    for old in ICLOUD.glob("hos_002_part03_rough_v*.mp4"):
        ...
        if old.exists() and not superseded.exists():
            ...
            # already superseded name taken — just remove the old watchable copy
            old.unlink()
    watch = ICLOUD / "WATCH_part03_v02.txt"
    watch.write_text(
        "WATCH THIS FILE ONLY (Part 03 rough v02 — hall continuity remint):\n"
        f"  {OUT.name}\n\n"
        "CoS / Ben gates for this cut:\n"
        "- HARD LOCK: plate 01 Karlsruhe hall DNA on EVERY scenery plate\n"
        "- Prefer I2V from plate-01 frames / same-room stills\n"
        "- Forbidden room swaps: blackboard / chandelier / fresco / model-town\n"
        "- All real Veo motion (no Ken Burns hero / no freeze-pad)\n"
        "- Animistry side labels (not center stamps)\n"
        "- Explorer garnish once ~28–36s (plate 05)\n"
        "- Part 01 v14 + Part 02 v06 stay LOCKED — do not remint\n\n"
        "Do not ping Ben. Reject with stills from THIS file only.\n"
        f"sha256={digest}\nbytes={bytes_n}\nduration={dur:.3f}\n"
    )
    (ICLOUD / "ZZ_OPEN_PART03_V02_ONLY.txt").write_text(
        "Part 03 current cut = hos_002_part03_rough_v02.mp4\n"
        "Part 01 v14 + Part 02 v06 are PASS locked keepers.\n"
    )
    (ICLOUD / "PART03_NEXT.txt").write_text(
        "Part 03 rough LANDED: hos_002_part03_rough_v02.mp4\n"
        "Watch WATCH_part03_v02.txt\n"
        "Hall continuity remint (UAT FAIL fix).\n"
