                    ...
                    item = page.evaluate(
                        """() => {
                          ...
                          for (const el of items) {
                            ...
                            let score=0;
                            if (/\\bmp4\\b/i.test(t)) score+=8;
                            if (/1080|720|full.?res|video/i.test(t)) score+=5;
                            if (/original/i.test(t) && !/1k|image|jpeg|png|still/i.test(t)) score+=3;
                            if (/1k|image|jpeg|png|still|gif|webp/i.test(t)) score-=8;
                            if (score>0) scored.push({score,t:t.slice(0,60),
                              x:r.x+r.width/2,y:r.y+r.height/2});
                          }
                          scored.sort((a,b)=>b.score-a.score);
                          return {pick: scored[0]||null, all: scored.slice(0,8)};
                        }"""
                    )
                    if item:
                        print(f"  menu all → {item.get('all')}", flush=True)
                        pick = item.get("pick") if isinstance(item, dict) else item
                        if pick:
                            print(f"  menu → {pick}", flush=True)
                            page.mouse.click(pick["x"], pick["y"])
                        else:
                            print("  no scored menu item — waiting for direct download", flush=True)
                    else:
