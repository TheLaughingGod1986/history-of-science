"""Assemble HOS 002 Part 04 rough v14 — Ben FAIL remint v15 — hair + CLEAN LIGHT + sharp late desks.

Parent FAIL: hos_002_part04_rough_v13.mp4
  sha256 27828f216c289d36aaeae7311c545cdf0fb829381ff110c528cc40008710cd6a
Single blocker: CLEAN LIGHT ~105s FAMILY FIRST side-label — chair-top flame.
Timeline: plate09b = 98.15–106.05 (fail still); plate 10 floating-cards ~105.7+.
UAT named plate 10; picture is 09b. Remint 09b only.

KEEP from v13: 04,05,06 Explorer,09,10,05b,07,08,08b,02b,07b (+ all non-09b).
...
RAW_V14 = PROJ / "04_Generated-Clips/part04/raw/v14_fast"
...
OUT = PROJ / "09_Final-Export/hos_002_part04_rough_v15.mp4"
...
REMINT_V15_IDS = {
    "06_explorer_leaves_gap",  # unfinished crown hair
    "09_risk_bet",  # Ben lava chair + ghost books (~95s A BET)
    "09b_risk_hold",  # lamp lava + late blur
    "10_family_before_weight",  # late blur / lamp
    "11_publish_gaps",  # unfinished late desk
    "11b_wait_and_hunt",  # unfinished late desk
}
REMINT_V14_IDS = set()  # superseded
KEEP_V13_IDS = {
...
def resolve_clip(pid: str) -> tuple[Path, str]:
    if pid in REMINT_V15_IDS:
        v15 = RAW_V15 / f"{pid}_v15.mp4"
        if v15.exists() and v15.stat().st_size >= 400_000:
            return v15, "v15_ben_fail_remint"
        raise SystemExit(f"missing reminted v15 plate {v15}")
...
def main() -> None:
    ...
    if not KEEP_V14_PARENT.exists():
        raise SystemExit(f"missing parent v14 {KEEP_V14_PARENT}")
    v14_sha = sha256(KEEP_V14_PARENT)
    if v14_sha != KEEP_V14_PARENT_SHA:
        raise SystemExit(
            f"STOP: v14 parent sha mismatch want {KEEP_V14_PARENT_SHA} got {v14_sha}"
        )
    print(f"KEEP v14 parent sha OK {v14_sha}", flush=True)

    if not KEEP_V13_PARENT.exists():
        raise SystemExit(f"missing parent v13 {KEEP_V13_PARENT}")
    v13_sha = sha256(KEEP_V13_PARENT)
    if v13_sha != KEEP_V13_PARENT_SHA:
        raise SystemExit(
            f"STOP: v13 parent sha mismatch want {KEEP_V13_PARENT_SHA} got {v13_sha}"
        )
    print(f"KEEP v13 parent sha OK {v13_sha}", flush=True)
    ...
    dur = probe(OUT)
    reminted = [pid for pid, src in zip(ordered_ids, sources) if src == "v15_ben_fail_remint"]
    ...
    meta_path = PROJ / "07_Edit-Project/part04_rough_v15_land_meta.json"
        ...
        json.dumps(
            {
                ...
                "duration_s": dur,
                "sha256": digest,
                "parent_v14": KEEP_V13_PARENT.name,
                "parent_v13_sha256": v13_sha,
                "parent_v12_sha256": v12_sha,
                "reminted_plates": reminted,
                "uat_named_plate": "10_family_before_weight",
                "picture_reminted_plate": "09b_risk_hold",
                "timeline_note": (
                    "UAT FAIL ~105s under FAMILY FIRST side-label is plate 09b "
                    "(~98.15–106.05). Plate 10 floating-cards starts ~105.7 — kept from v13."
                ),
                "plate09b_sha256": sha256(RAW_V14 / "09b_risk_hold_v14.mp4"),
                "plate09b_method": (
                    "Flow Veo try2 rejected (flame) → clean-chair-lock composite: "
                    "ok_abet DNA start locks chair/lamp/shelves; desk cards from try2 motion"
                ),
                ...
                "watch": "WATCH_part04_v15.txt",
                "doc": "PART04_V15_BEN_FAIL.md",
                "status": (
    ...
    watch = ICLOUD / "WATCH_part04_v15.txt"
    watch.write_text(
        "WATCH THIS FILE ONLY (Part 04 rough v14 — CLEAN GLOW chair remint):\n"
        f"  {OUT.name}\n\n"
        "CoS / Picture→UAT gates for this cut:\n"
        "- Parent FAIL v13 sha 27828f216c289d36… — single blocker CLEAN LIGHT ~105s.\n"
        "- UAT named plate 10; picture blocker is 09b_risk_hold (~98–106s under FAMILY FIRST).\n"
        "- Reminted ONLY 09b (clean soft rectangular backrest glow, ZERO chair-top flame).\n"
        "- KEEP from v13: 04,05,06 Explorer,09,10,05b,07,08,08b,02b,07b (+ all non-09b).\n"
        "- Spot ~102–110s: soft panel glow ONLY — NO flame / fire wisp / lamp smoke.\n"
        "- Cards readable (H/C/O/N/Li). Indoor wood/bookcase. No blue fills/roofs.\n"
        ...
        f"keep_p03_sha={p03_sha}\n"
        f"parent_v13_sha={v13_sha}\n"
        f"reminted={','.join(reminted)}\n"
        f"plate09b_picture={plate09b_start:.3f}-{plate09b_end:.3f}\n"
        f"sha256={digest}\nbytes={bytes_n}\nduration={dur:.3f}\n"
    )
    (PROJ / "07_Edit-Project/WATCH_part04_v15.txt").write_text(watch.read_text())
    (ICLOUD / "ZZ_OPEN_PART04_V14_ONLY.txt").write_text(
        "Part 04 current cut = hos_002_part04_rough_v14.mp4\n"
        "Part 01–03 LOCKED keepers.\n"
        "v13 FAIL (chair-top flame @ ~105s) → v14 09b clean-glow remint — watch v14 only.\n"
    )
    (ICLOUD / "PART04_NEXT.txt").write_text(
        "Part 04 rough remint LANDED: hos_002_part04_rough_v14.mp4\n"
        "Watch WATCH_part04_v15.txt · PART04_V15_BEN_FAIL.md\n"
        "Scores → CoS. Do not declare PASS. Do not ping Ben.\n"
    )
