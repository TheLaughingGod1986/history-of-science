#!/usr/bin/env python3
"""Dedicated 11_publish_gaps try8B submit on live Mini CDP (start 2/2 same DNA).

Parent try8 start1 SELF_REJECT sha 781b80b0… — readable text invent on grid
(~SECURITY/RESULT) while CLEAN LIGHT + flasks CLEARED.

Same DNA: cropL40 zerolamp moonwin. HARD ban any printed words / captions / UI.
Paint banned. No assemble. No 11b/09b/06. Max 2 same-DNA then STOP_TO_COS.
"""
...
OUT = PROJ / "07_Edit-Project/_qa_part04_v20_flow/submit_11_publish_gaps_try8B.json"
...
PROMPT = (
    ...
    "Plate 11_publish_gaps try8B — PUBLISH THE GAPS (same DNA as try8; HARD TEXT LOCK). "
    ...
    "TEXT / CAPTION LOCK (HARD for full 8s — try8 FAIL): the grid board and every paper stay BLANK. "
    "ZERO printed words, ZERO letters, ZERO captions, ZERO UI, ZERO watermarks, ZERO logos, "
    "ZERO 'SECURITY', ZERO 'RESULT', ZERO 'PUBLISH', ZERO any readable text anywhere in frame. "
    "Blank cream tiles only — no symbols. "
...
def bump_start_counter() -> int:
    ...
    if n > MAX_STARTS:
        raise SystemExit(
            f"STOP_TO_COS: try8 DNA already used {n - 1}/{MAX_STARTS} Flow starts — do not mint further"
        )
...
def main() -> None:
    ...
    with sync_playwright() as p:
        ...
        tmp = Path("/tmp/hos_v20_11_try8B_stub.mp4")
        if tmp.exists():
            tmp.unlink()
        info: dict = {}
        try:
        ...
        out = {
            ...
            "start_dna": (
                "try8B = try8 zerolamp moonwin SAME DNA + HARD TEXT LOCK "
                "(blank grid/papers; zero SECURITY/RESULT/captions); "
                "CLEAN=window/moon/off-world bounce; dull flasks; plate-library 4b8ed25"
            ),
            ...
            "parent_try8": {
                "verdict": "SELF_REJECT",
                "sha256": "781b80b0df0c144ebda0cdd33d648fe061920d65985b9951e829ba8b4c64837c",
                "blocker": "readable text invent on grid (~SECURITY/RESULT)",
                "cleared": "lamp invent, flask molten/desk-glow, soft gap glow OK, continuous motion",
            },
            "parent_06": {
                "verdict": "UAT_PASS",
