#!/usr/bin/env python3
"""Assemble HOS 002 Part 04 rough v09 — Explorer HAT-FREE + colour DNA remint.

Parent KEEP: hos_002_part04_rough_v08.mp4 sha 8cafb379…
  - REMINT ONLY: 06_explorer_leaves_gap (v09_fast)
  - KEEP: same plate sources as v08 timeline (v07 sky scrub 05/09/09b, v06 desk 02b, v01 rest)
  - P01–P03 FROZEN
...
RAW_V09 = PROJ / "04_Generated-Clips/part04/raw/v09_fast"
...
OUT = PROJ / "09_Final-Export/hos_002_part04_rough_v09.mp4"
...
REMINT_V09_IDS = {"06_explorer_leaves_gap"}
...
def resolve_clip(pid: str) -> tuple[Path, str]:
    if pid in REMINT_V09_IDS:
        ...
        if v09.exists() and v09.stat().st_size >= 400_000:
            return v09, "v09_veo_hatfree_colour"
...
def main() -> None:
    ...
    dur = probe(OUT)
    reminted = [pid for pid, src in zip(ordered_ids, sources) if src == "v09_veo_hatfree_colour"]
    plate06 = RAW_V09 / "06_explorer_leaves_gap_v09.mp4"
    ...
    meta_path = PROJ / "07_Edit-Project/part04_rough_v09_land_meta.json"
        ...
        json.dumps(
            {
                ...
                "method": (
                    "Veo 3.1 Fast I2V — Explorer hat-free + colour DNA remint only "
                ...
                "watch": "WATCH_part04_v09.txt",
                "note": (
                    "v09: remint ONLY 06_explorer_leaves_gap — NO hat + colour DNA lock "
    ...
    watch = ICLOUD / "WATCH_part04_v09.txt"
    watch.write_text(
        "WATCH THIS FILE ONLY (Part 04 rough v09 — Explorer HAT-FREE + colour DNA):\n"
        f"  {OUT.name}\n\n"
    ...
    (PROJ / "07_Edit-Project/WATCH_part04_v09.txt").write_text(watch.read_text())
    (ICLOUD / "ZZ_OPEN_PART04_V09_ONLY.txt").write_text(
        "Part 04 current cut = hos_002_part04_rough_v09.mp4\n"
        "Part 01–03 LOCKED keepers.\n"
        "v08 UAT FAIL hat+colour → v09 hat-free colour DNA — watch v09 only.\n"
    )
    (ICLOUD / "PART04_NEXT.txt").write_text(
        "Part 04 rough remint LANDED: hos_002_part04_rough_v09.mp4\n"
        "Watch WATCH_part04_v09.txt\n"
