#!/usr/bin/env python3
"""Assemble HOS 002 Part 04 rough v07 — Explorer garnish remint (toy-scale teal).
...
RAW_V08 = PROJ / "04_Generated-Clips/part04/raw/v08_fast"
...
OUT = PROJ / "09_Final-Export/hos_002_part04_rough_v08.mp4"
...
REMINT_V08_IDS = {"06_explorer_leaves_gap"}
...
def main() -> None:
    ...
    dur = probe(OUT)
    reminted = [pid for pid, src in zip(ordered_ids, sources) if src == "v08_veo_garnish"]
    print(
        f"LANDED {OUT}\n"
        f"  path={OUT}\n"
        f"  bytes={bytes_n}\n"
        ...
        f"  parent_v06_sha={v06_sha}\n",
        flush=True,
    )

    meta_path = PROJ / "07_Edit-Project/part04_rough_v08_land_meta.json"
        ...
        json.dumps(
            {
                ...
                "method": "Veo 3.1 Fast I2V — Explorer toy-scale garnish remint only",
                ...
                "watch": "WATCH_part04_v08.txt",
                "note": (
                    "v08: remint ONLY 06_explorer_leaves_gap to toy-scale teal garnish "
                    "(P01/P03 house). KEEP v07 sky scrub on 05/09/09b + v06 desk 02b. "
    ...
    watch = ICLOUD / "WATCH_part04_v08.txt"
    watch.write_text(
        "WATCH THIS FILE ONLY (Part 04 rough v07 — Explorer garnish remint (toy-scale teal)):\n"
        f"  {OUT.name}\n\n"
        ...
        "- Reminted: 06_explorer_leaves_gap ONLY (Veo Fast I2V/T2V garnish). KEEP v07 sky scrub 05/09/09b + v06 desk 02b.\n"
        "- Spot Explorer ~45–53s: toy-scale teal garnish (profile/OTS), NOT face-hero.\n"
        ...
        f"plate09_picture={plate09_start:.3f}-{plate09b_end:.3f}\n"
        f"plate09b_picture={plate09b_start:.3f}-{plate09b_end:.3f}\n"
        f"sha256={digest}\nbytes={bytes_n}\nduration={dur:.3f}\n"
    )
    # Also land watch next to Final-Export for repo trail
    (PROJ / "07_Edit-Project/WATCH_part04_v08.txt").write_text(watch.read_text())
    (ICLOUD / "ZZ_OPEN_PART04_V08_ONLY.txt").write_text(
        "Part 04 current cut = hos_002_part04_rough_v08.mp4\n"
    ...
    (ICLOUD / "PART04_NEXT.txt").write_text(
        "Part 04 rough remint LANDED: hos_002_part04_rough_v08.mp4\n"
        "Watch WATCH_part04_v08.txt\n"
