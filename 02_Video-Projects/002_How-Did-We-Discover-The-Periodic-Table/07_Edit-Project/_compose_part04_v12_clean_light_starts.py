...
OUT_DIR = PROJ / "04_Generated-Clips/part04/refs/v12_start_frames"
...
def soft_chair_glow(im: Image.Image) -> Image.Image:
    """Soft Empty-Chairs warm glow — NOT fire."""
    out = im.convert("RGBA")
    w, h = out.size
    overlay = Image.new("RGBA", (w, h), (0, 0, 0, 0))
    d = ImageDraw.Draw(overlay)
    d.rounded_rectangle(
        (int(w * 0.38), int(h * 0.22), int(w * 0.62), int(h * 0.55)),
        radius=28,
        fill=(255, 210, 120, 48),
    )
...
def compose_desk(
    ...
    *,
    ...
    out = OUT_DIR / f"{pid}_start_v12.jpg"
    im.save(out, quality=94)
    return out
...
def main() -> None:
    ...
    # Remint desks only — KEEP 06_explorer_leaves_gap untouched
    jobs = [
        (
            "04_sort_atomic_weight",
            RAW_V01 / "04_sort_atomic_weight_v01.mp4",
            2.0,
            [
                (0.28, 0.42, 0.40, 0.62),
                (0.40, 0.40, 0.52, 0.60),
        ...
        (
            "05_columns_families",
            RAW_V07 / "05_columns_families_v07.mp4",
            3.0,
            [
                (0.22, 0.48, 0.34, 0.72),
                (0.36, 0.46, 0.48, 0.70),
        ...
        (
            "05b_families_settle",
            RAW_V11 / "05b_families_settle_v11.mp4",
            2.0,
            [
                (0.34, 0.55, 0.48, 0.82),
                (0.48, 0.55, 0.62, 0.82),
        ...
        (
            "07_eka_placeholders",
            RAW_V11 / "07_eka_placeholders_v11.mp4",
            2.0,
            [
                (0.30, 0.55, 0.48, 0.82),
                (0.15, 0.45, 0.28, 0.68),
        ...
        (
            ...
            RAW_V01 / "08_prediction_navigation_v01.mp4",
            2.5,
            [
                (0.12, 0.48, 0.26, 0.70),
                (0.28, 0.62, 0.40, 0.80),
        ...
        (
            "08b_navigation_walk",
            RAW_V01 / "08b_navigation_walk_v01.mp4",
            3.0,
            [
                (0.38, 0.48, 0.55, 0.72),
                (0.55, 0.52, 0.68, 0.72),
        ...
        (
            ...
            RAW_V01 / "10_family_before_weight_v01.mp4",
            2.5,
            [
                (0.28, 0.35, 0.42, 0.58),
                (0.42, 0.32, 0.56, 0.55),
    ...
    meta = {
        ...
        "keep": ["06_explorer_leaves_gap", "02b_cards_sixty_three", "P01-P03"],
        "locks": [
        ...
        "parent_fail": "hos_002_part04_rough_v11.mp4",
        "parent_sha": "96a0f41e53987e090e5eee6af29d3abda8de3c30b326584193c10847d1ebd739",
    }
