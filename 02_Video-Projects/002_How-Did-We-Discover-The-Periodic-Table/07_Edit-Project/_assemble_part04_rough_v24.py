#!/usr/bin/env python3
"""Assemble HOS 002 Part 04 rough v24 — A BET FAIL/WIN remints.

Parent: hos_002_part04_rough_v23.mp4
  sha256 7beae93d10a87066731ae58bc39039eabda625154aa7f0aa3e7c4d9f6e5e5b5f

Ben UAT 10 Sep 2026 on v23: Explorer + music good. A BET moon-desk still-life
repeats 3–4 times; needs more animation and must show the VO (if nothing
arrives = imaginary metals; if something matching arrives = machine for the
unknown).

v24: remint ONLY 08b_navigation_walk (FAIL, glow dies, seat stays empty) and
09_risk_bet (WIN, cream card/ingot locks into the vacant slot). KEEP 09b try14.
T2V Quality, no moon-desk start attach. NO PAINT. Scores → CoS. Do not declare PASS.
"""
...
OUT = PROJ / "09_Final-Export/hos_002_part04_rough_v24.mp4"
...
def main() -> None:
    ...
    meta_path = PROJ / "07_Edit-Project/part04_rough_v24_land_meta.json"
        ...
        json.dumps(
            {
                ...
                "watch": "WATCH_part04_v24.txt",
                "ben_uat_v23": {
                    "abet": "moon-desk still-life repeated 3-4x; remint 08b FAIL + 09 WIN",
                    "keep_09b": "try14 three glowing vacant chairs",
                },
                "status": (
                    "LANDED_FOR_UAT — Ben fix: A BET FAIL/WIN remints; "
                    "do not declare PASS"
                ),
            },
            indent=2,
    ...
    watch_body = (
        "WATCH THIS FILE ONLY (Part 04 rough v24 — A BET FAIL/WIN remints):\n"
        f"  {OUT.name}\n\n"
        "Ben UAT fix vs v23:\n"
        "- A BET stretch no longer repeats the moon-desk still-life 3–4 times.\n"
        "- 08b FAIL: empty chair glow dies; vacant seat stays empty (imaginary metals).\n"
        "- 09 WIN: matching cream card/ingot locks into the vacant slot (bet pays).\n"
        "- KEEP 09b try14 three glowing vacant chairs.\n"
        "- Explorer try8 + looped music bed unchanged from v23.\n\n"
        ...
        "- Scrub A BET ~83.5–100s: FAIL then WIN then 09b chairs — not the same still-life.\n"
    ...
    watch = ICLOUD / "WATCH_part04_v24.txt"
    watch.write_text(watch_body)
    (PROJ / "07_Edit-Project/WATCH_part04_v24.txt").write_text(watch_body)
    (ICLOUD / "ZZ_OPEN_PART04_V24_ONLY.txt").write_text(
        "Part 04 current cut = hos_002_part04_rough_v24.mp4\n"
        "Part 01–03 LOCKED keepers.\n"
        "Ben fix: A BET FAIL/WIN remints — watch v24 only.\n"
    )
    (ICLOUD / "PART04_NEXT.txt").write_text(
        "Part 04 rough assemble LANDED: hos_002_part04_rough_v24.mp4\n"
        "Watch WATCH_part04_v24.txt\n"
        "Ben UAT: A BET FAIL/WIN remints. Scores → CoS. Do not declare PASS.\n"
    )
