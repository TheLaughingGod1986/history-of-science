#!/usr/bin/env python3
"""Assemble HOS 002 Part 04 rough v10 — Explorer PROFILE/BACK pose remint (colour DNA keep).

Parent KEEP: hos_002_part04_rough_v10.mp4 sha 8cafb379…
  - REMINT ONLY: 06_explorer_leaves_gap (v10_fast)
  - KEEP: same plate sources as v09 timeline (v07 sky scrub 05/09/09b, v06 desk 02b, v01 rest)
  - P01–P03 FROZEN
...
OUT = PROJ / "09_Final-Export/hos_002_part04_rough_v10.mp4"
...
REMINT_V10_IDS = {"06_explorer_leaves_gap"}
KEEP_V07_IDS = {"05_columns_families", "09_risk_bet", "09b_risk_hold"}
...
def resolve_clip(pid: str) -> tuple[Path, str]:
    if pid in REMINT_V10_IDS:
        v09 = RAW_V10 / f"{pid}_v10.mp4"
        if v09.exists() and v09.stat().st_size >= 400_000:
            return v09, "v10_veo_profile_pose"
        raise SystemExit(f"missing reminted v10 plate {v09}")
    if pid in KEEP_V07_IDS:
        v07 = RAW_V07 / f"{pid}_v07.mp4"
        if v07.exists() and v07.stat().st_size >= 400_000:
            return v07, "v07_sky_keep"
        raise SystemExit(f"missing v07 KEEP plate {v07}")
...
def main() -> None:
    ...
    dur = probe(OUT)
    reminted = [pid for pid, src in zip(ordered_ids, sources) if src == "v10_veo_profile_pose"]
    plate06 = RAW_V10 / "06_explorer_leaves_gap_v10.mp4"
    ...
    meta_path = PROJ / "07_Edit-Project/part04_rough_v10_land_meta.json"
        ...
        json.dumps(
            {
                ...
                "method": (
                    "Veo 3.1 Fast I2V — Explorer profile/back pose + colour DNA keep remint only "
                    "(fair warm skin, dark teal coat, bare head)"
                ),
                ...
                "watch": "WATCH_part04_v10.txt",
                "note": (
                    "v10: remint ONLY 06_explorer_leaves_gap — PROFILE/BACK pose + colour DNA keep "
                    "(sheet/Germs/P03). KEEP v08 timeline otherwise. Scores → CoS only. "
                    "Do not declare PASS. Do not ping Ben."
                ),
            },
            indent=2,
        )
    ...
    watch = ICLOUD / "WATCH_part04_v10.txt"
    watch.write_text(
        "WATCH THIS FILE ONLY (Part 04 rough v09 — Explorer PROFILE/BACK + colour DNA):\n"
        f"  {OUT.name}\n\n"
    ...
    (PROJ / "07_Edit-Project/WATCH_part04_v10.txt").write_text(watch.read_text())
    (ICLOUD / "ZZ_OPEN_PART04_V10_ONLY.txt").write_text(
        "Part 04 current cut = hos_002_part04_rough_v10.mp4\n"
        "Part 01–03 LOCKED keepers.\n"
        "v08 UAT FAIL hat+colour → v09 hat-free colour DNA — watch v09 only.\n"
    )
    (ICLOUD / "PART04_NEXT.txt").write_text(
        "Part 04 rough remint LANDED: hos_002_part04_rough_v10.mp4\n"
        "Watch WATCH_part04_v10.txt\n"
