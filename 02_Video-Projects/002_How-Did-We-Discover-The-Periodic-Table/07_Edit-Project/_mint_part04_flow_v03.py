#!/usr/bin/env python3
"""Part 04 Flow Veo 3.1 Fast partial remint v03 — scrub house props on books.

Showrunner PART04_V03_SCRUB_HOUSE_PROPS_BOOKS.md (parent v02 sha d5377dad…):
  - REMINT: 02b_cards_sixty_three (REQUIRED ~19–21s glowing yellow houses on books)
  - Spot-checked 03 / 04 — clean; do NOT remint 05/06 window KEEP
  - Books OK; no glowing yellow houses / house-icon tokens / model-town toys
...
RAW = PROJ / "04_Generated-Clips/part04/raw/v03_fast"
META = PROJ / "07_Edit-Project/part04_mint_flow_v03_meta.json"
...
REJECT = (
    "HARD REJECT: glowing yellow house props on books; house-icon tokens; "
    "miniature model-town toys on the desk; blue/yellow toy building blocks; "
    "glowing house-blocks; model-town outside the window; Ken Burns only; "
    "Orbit robot; photoreal; layout reset away from desk DNA."
)

DESK_PROP_LOCK = (
    "Leather-bound book stack may stay on the desk — but the book tops must be "
    "CLEAN leather only. ZERO glowing yellow houses, ZERO house-shaped tokens, "
    "ZERO house-icon blocks, ZERO miniature town props, ZERO yellow glowing "
    "toys on or near the books. Cream blank cards / papers / opaque vessels OK."
)
...
PROMPT_02B = (
    ...
    "leather books). Cream blank cards keep settling / gentle shuffle across the "
    "honey wood — denser stack as if counting known elements. Continuous card "
    "motion. Soft lamp. Night window with sky only. "
    f"{DESK_PROP_LOCK} "
...
T2V_02B = (
    ...
    "cards settling in a dense stack, leather books with CLEAN tops (no toys), "
    "lab vessels. Continuous gentle card shuffle. Night sky window only. Silent. "
    "No people. " + STYLE + " " + REJECT + " " + DESK_PROP_LOCK
)
...
START_FRAMES = {
    "02b_primary": STILLS / "02b_books_scrub_i2v.jpg",
    "02b_alt": STILLS / "02b_books_scrub_alt.jpg",
    "02b_t1": STILLS / "02b_books_scrub_t1.jpg",
    "02b_rough": STILLS / "02b_books_scrub_from_rough.jpg",
}
...
def resolve_start_frame(pid: str) -> Path:
    ...
    keys = ["02b_primary", "02b_alt", "02b_t1", "02b_rough"]
    for key in keys:
...
def alt_starts_for(pid: str, start: Path) -> list[Path]:
    if pid != "02b_cards_sixty_three":
        return []
    keys = ["02b_alt", "02b_t1", "02b_rough", "02b_primary"]
    out: list[Path] = []
    for key in keys:
        path = START_FRAMES[key]
...
def main() -> None:
    if _truthy("HOS_FLOW_SKIP_I2V"):
        print(
            "  WARN: unsetting HOS_FLOW_SKIP_I2V — v03 prefers scrubbed I2V",
            flush=True,
        )
    ...
    if bad:
        raise SystemExit(f"STOP: refuse remint via v03: {sorted(bad)}")
    ...
    meta: dict = {
        ...
        "raw": str(RAW),
        "continuity": "scrub_house_props_books_v03",
        "parent_v02_sha": "d5377dadc6c862c47348f79ad27b377f63c2b09b0b9607824e0a3c0383350f6c",
        "only": sorted(only),
        "plates": [],
    }
    if META.exists():
    ...
    want = len(plates)
    print(f"OK part 04 Flow mint v03 finished ok={ok} want={want}", flush=True)
    if ok < want:
        sys.exit(2)
