#!/usr/bin/env python3
"""Dedicated 09b_risk_hold try9B submit on live Mini CDP — cardless moon DNA, CLEAN LAMP (no lava drip)."""
...
LOCK = PROJ / "04_Generated-Clips/part04/refs/v20_start_frames/09b_risk_hold_start_v20_try9_cardless.jpg"
...
OUT = PROJ / "07_Edit-Project/_qa_part04_v20_flow/submit_09b_risk_hold_try9B.json"
...
def main() -> None:
    ...
    if LOCK.exists() and s != sha(LOCK):
        raise SystemExit("ABORT: active start != try9 cardless nodrip lock")
    ...
    with sync_playwright() as p:
        ...
        tmp = Path("/tmp/hos_v20_09b_try9B_stub.mp4")
        if tmp.exists():
            tmp.unlink()
        info: dict = {}
        try:
        ...
        out = {
            ...
            "start": str(START),
            "start_sha256": s,
            "start_dna": "try9B alternate cardless KEEP from v01 empty-chair moon t=0.2 (nodrip); NOT try8 KEEP cf182dc drip-baked; NOT flat 2a651abc",
            "model": MODEL,
