#!/usr/bin/env python3
"""Plate UAT for 10_family_before_weight try1 — continuous contact + lamp crops.
...
# Upper-left / top invent zones where v21 FAMILY FIRST lava lived.
...
def judge(mp4: Path, out_dir: Path, try_id: str = "1") -> dict:
    ...
    for d in (frames_dir, lamp_dir, fixture_dir):
    ...
    for t in TIMES:
        ...
        still = out_dir / f"10_family_before_weight_t{t:.1f}.jpg"
        ...
        # Extra left-upper fixture contact (05 try2A invent zone)
        save_box((0.00, 0.00, 0.35, 0.35), fixture_dir / f"t{t:.1f}_left_upper.jpg")
    ...
    contact = out_dir / f"10_family_before_weight_contact_continuous_try{try_id}.jpg"
    contact_strip(frame_paths, contact, [f"t{t}" for t in TIMES])
    lamp_contact = out_dir / f"10_family_before_weight_lamp_contact_try{try_id}.jpg"
    contact_strip(lamp_paths, lamp_contact, [f"t{t}" for t in TIMES])
    fixture_paths = sorted(fixture_dir.glob("t*_left_upper.jpg"))
    fixture_contact = out_dir / f"10_family_before_weight_fixture_contact_try{try_id}.jpg"
    if fixture_paths:
        contact_strip(fixture_paths, fixture_contact, [p.stem.split("_")[0] for p in fixture_paths])
    ...
    report = {
        ...
        "frames": results,
        "contact_continuous": contact.name,
        "lamp_contact": lamp_contact.name,
        "fixture_contact": fixture_contact.name if fixture_paths else None,
        "assemble": False,
    ...
    (out_dir / f"plate_qa_10_try{try_id}.json").write_text(json.dumps(report, indent=2) + "\n")
...
def main() -> None:
    ...
    judge(args.mp4, args.out, args.try_id)
