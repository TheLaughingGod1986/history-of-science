#!/usr/bin/env python3
"""Dedicated 06_explorer_leaves_gap try2 submit on live Mini CDP.

Same DNA as try1 (OTS/back + 1869 card-desk + no practical lamp) with LOCKED BACK/OTS:
Explorer NEVER turns to face camera (try1 SELF_REJECT = glasses miss on face-turn).
...
OUT = PROJ / "07_Edit-Project/_qa_part04_v20_flow/submit_06_explorer_leaves_gap_try2.json"
...
PROMPT = (
    ...
    "Plate 06_explorer_leaves_gap try2 — EMPTY SEATS garnish, SAME DNA as try1 with HARD camera lock. "
    ...
    "(try1 failed when face turned without house glasses — this take keeps face away.) "
    ...
    "leaving a soft glowing vacant seat / gap on the desk — soft warm glow only, "
    ...
    "face-turn toward camera, front-facing Explorer hero. "
...
def main() -> None:
    ...
    with sync_playwright() as p:
        ...
        tmp = Path("/tmp/hos_v20_06_try2_stub.mp4")
        if tmp.exists():
            tmp.unlink()
        info: dict = {}
        try:
        ...
        out = {
            ...
            "start_dna": (
                "try2 same DNA as try1 + LOCKED BACK/OTS full 8s (never face-turn); "
                "CLEAN=window/off-world bounce ONLY; ZERO practical lamp; plate-library 4b8ed25"
            ),
            ...
            "paint": "banned",
            "assemble": False,
            "parent_try1": "SELF_REJECT glasses-when-visible on face-turn; lamp CLEAR",
        }
