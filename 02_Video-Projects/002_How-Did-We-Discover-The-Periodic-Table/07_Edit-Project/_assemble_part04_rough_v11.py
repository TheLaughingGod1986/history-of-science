"""Assemble HOS 002 Part 04 rough v11 — Ben FAIL remint (glasses / clean lamp / written cards).

Parent FAIL: hos_002_part04_rough_v10.mp4 sha eab7f8ec…
  - REMINT: 06, 05b, 07, 07b, 09, 09b (v11_fast)
  - KEEP: 02b v06 desk; 05 v07; all other prior sources
  - P01–P03 FROZEN
...
OUT = PROJ / "09_Final-Export/hos_002_part04_rough_v11.mp4"
...
KEEP_V07_IDS = {"05_columns_families"}  # 09/09b reminted in v11
...
def resolve_clip(pid: str) -> tuple[Path, str]:
    if pid in REMINT_V11_IDS:
        v11 = RAW_V11 / f"{pid}_v11.mp4"
        if v11.exists() and v11.stat().st_size >= 400_000:
            return v11, "v11_ben_fail_remint"
        raise SystemExit(f"missing reminted v11 plate {v11}")
...
def main() -> None:
    ...
    dur = probe(OUT)
    reminted = [pid for pid, src in zip(ordered_ids, sources) if src == "v11_ben_fail_remint"]
    plate06 = RAW_V11 / "06_explorer_leaves_gap_v11.mp4"
    ...
    meta_path = PROJ / "07_Edit-Project/part04_rough_v11_land_meta.json"
        ...
        json.dumps(
            {
                ...
                "parent_v10_sha256": v10_sha,
                "reminted_plates": reminted,
                "plate06_sha256": plate06_sha,
                "method": (
                    "Veo 3.1 Fast I2V — Ben FAIL remint: glasses + clean lamp + written cards "
                    "(06/05b/07/07b/09/09b)"
                ),
                ...
                "note": (
                    "v11: Ben FAIL remint — glasses + clean lamp + written cards on 06/05b/07/07b/09/09b. "
                    "KEEP 02b + other clean plates. Scores → CoS only. Do not declare PASS. Do not ping Ben."
                ),
            },
            indent=2,
        )
    ...
    watch = ICLOUD / "WATCH_part04_v11.txt"
    watch.write_text(
    ...
    (PROJ / "07_Edit-Project/WATCH_part04_v11.txt").write_text(watch.read_text())
