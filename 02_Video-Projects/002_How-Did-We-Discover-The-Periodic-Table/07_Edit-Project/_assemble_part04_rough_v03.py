#!/usr/bin/env python3
"""Assemble HOS 002 Part 04 rough v03 — scrub glowing house props on books.

Parent: hos_002_part04_rough_v02.mp4 sha d5377dad…
  - REMINT ONLY 02b_cards_sixty_three (desk house props ~19–21s)
  - KEEP v02's 05/06 sky fills + all other v01 plates, VO, bed, labels, PLATE_ORDER
...
RAW_V03 = PROJ / "04_Generated-Clips/part04/raw/v03_fast"
LABEL_DIR = PROJ / "04_Generated-Clips/part04/refs/v03_side_labels"
VO = PROJ / "02_Voiceover/part04_empty_chairs_v01.wav"
BED = PROJ / "05_Music/hos_002_part01_curious_workshop_v02_norm.wav"
OUT = PROJ / "09_Final-Export/hos_002_part04_rough_v03.mp4"
KEEP_V02 = PROJ / "09_Final-Export/hos_002_part04_rough_v02.mp4"
KEEP_V02_SHA = "d5377dadc6c862c47348f79ad27b377f63c2b09b0b9607824e0a3c0383350f6c"
...
REMINT_V03_IDS = {"02b_cards_sixty_three"}
...
def main() -> None:
    ...
    if not KEEP_V02.exists():
        raise SystemExit(f"missing parent v02 {KEEP_V02}")
    v02_sha = sha256(KEEP_V02)
    if v02_sha != KEEP_V02_SHA:
        raise SystemExit(
            f"STOP: v02 parent sha mismatch want {KEEP_V02_SHA} got {v02_sha}"
        )
    print(f"KEEP v02 parent sha OK {v02_sha}", flush=True)
    ...
    dur = probe(OUT)
    reminted = [pid for pid, src in zip(ordered_ids, sources) if src == "v03_scrub"]
    ...
    print(
        ...
        f"  parent_v02_sha={v02_sha}\n",
        flush=True,
    )

    meta_path = PROJ / "07_Edit-Project/part04_rough_v03_land_meta.json"
        ...
        json.dumps(
            {
                ...
                "note": (
                    "Scrub glowing yellow house props on books ~19–21s "
                    "(02b). KEEP v02 05/06 sky. Scores → CoS only. Do not declare PASS."
                ),
            },
            indent=2,
        )
    ...
    watch = ICLOUD / "WATCH_part04_v03.txt"
    watch.write_text(
        "WATCH THIS FILE ONLY (Part 04 rough v03 — scrub house props on books):\n"
        f"  {OUT.name}\n\n"
        "CoS / Picture→UAT gates for this cut:\n"
        "- Parent v02 FAIL ~19–21s: glowing yellow house / model-town props on books "
        "under ATOMIC WEIGHT.\n"
        "- Reminted: 02b_cards_sixty_three only (03/04 spot-clean; 05/06 sky KEEP).\n"
        "- Whole-cut scan: zero house props on desk OR window.\n"
        "- KEEP: ~40–45s night sky only · Explorer teal · VO/labels · desk DNA · "
        "same PLATE_ORDER.\n"
        "- Real Veo Fast remint. Continuous motion. No Ken Burns.\n"
    ...
    (ICLOUD / "ZZ_OPEN_PART04_V03_ONLY.txt").write_text(
        "Part 04 current cut = hos_002_part04_rough_v03.mp4\n"
        "Part 01 v14 + Part 02 v06 + Part 03 v09 are LOCKED keepers.\n"
        "v02 was UAT FAIL (house props on books) — watch v03 only.\n"
    )
    (ICLOUD / "PART04_NEXT.txt").write_text(
        "Part 04 rough remint LANDED: hos_002_part04_rough_v03.mp4\n"
        "Watch WATCH_part04_v03.txt\n"
