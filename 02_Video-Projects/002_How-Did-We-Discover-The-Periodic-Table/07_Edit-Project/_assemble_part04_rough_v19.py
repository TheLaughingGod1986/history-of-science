#!/usr/bin/env python3
"""Assemble HOS 002 Part 04 rough v19 — Flow Ultra remint (bible 43d9405).

Parent FAIL: hos_002_part04_rough_v18.mp4
  sha256 05fb0a2b34dba1a8347ab74b2abc987fd406bef356b82b55f1a0e8ddb88cc6b5
UAT HARD FAIL: PUBLISH ~118–127 continuous playback horizontal ghost/jitter (paint fallback).
...
RAW_V19 = PROJ / "04_Generated-Clips/part04/raw/v19_fast"
...
OUT = PROJ / "09_Final-Export/hos_002_part04_rough_v19.mp4"
PARENT_V18 = PROJ / "09_Final-Export/hos_002_part04_rough_v18.mp4"
...
def resolve_clip(pid: str) -> tuple[Path, str]:
    if pid in REMINT_V19_IDS:
        v19 = RAW_V19 / f"{pid}_v19.mp4"
        # Flow gallery mp4 must be real (~≥400KB). Refuse paint-tiny leftovers.
        if v19.exists() and v19.stat().st_size >= 400_000:
            return v19, "v19_flow_ultra_remint"
        raise SystemExit(f"missing reminted Flow v19 plate {v19}")
...
def main() -> None:
    ...
    v18_sha = require_sha(PARENT_V18, PARENT_V18_SHA, "v18 parent")
    ...
    dur = probe(OUT)
    reminted = [pid for pid, src in zip(ordered_ids, sources) if src == "v19_flow_ultra_remint"]
    plate06 = RAW_V19 / "06_explorer_leaves_gap_v19.mp4"
    ...
    plate11 = RAW_V19 / "11_publish_gaps_v19.mp4"
    plate11_sha = sha256(plate11) if plate11.exists() else None
    plate11b = RAW_V19 / "11b_wait_and_hunt_v19.mp4"
    ...
    flow_meta_path = PROJ / "07_Edit-Project/part04_mint_flow_v19_meta.json"
    flow_jobs = {}
    if flow_meta_path.exists():
        try:
            ...
            flow_jobs = {}
    meta_path = PROJ / "07_Edit-Project/part04_rough_v19_land_meta.json"
        ...
        json.dumps(
            {
                ...
                "sha256": digest,
                "bible_main": "43d9405",
                "parent_v18": PARENT_V18.name,
                "parent_v18_sha256": v18_sha,
                ...
                "plates": {
                    pid: {
                        "path": str((RAW_V19 / f"{pid}_v19.mp4").relative_to(PROJ)),
                        "sha256": plate_sha[pid],
                        "bytes": (RAW_V19 / f"{pid}_v19.mp4").stat().st_size,
                    }
                    for pid in reminted
                },
                "methods": methods,
                ...
                "watch": "WATCH_part04_v19.txt",
                "doc": "PART04_V19_FLOW.md",
                "status": (
    ...
    watch = ICLOUD / "WATCH_part04_v19.txt"
    watch.write_text(
        "WATCH THIS FILE ONLY (Part 04 rough v19 — Flow Ultra remint):\n"
        f"  {OUT.name}\n\n"
    ...
    (PROJ / "07_Edit-Project/WATCH_part04_v19.txt").write_text(watch.read_text())
    (ICLOUD / "ZZ_OPEN_PART04_V19_ONLY.txt").write_text(
        "Part 04 current cut = hos_002_part04_rough_v19.mp4\n"
        "Part 01–03 LOCKED keepers.\n"
        "v18 paint FAIL → v19 Flow Ultra remint — watch v19 only.\n"
    )
    (ICLOUD / "PART04_NEXT.txt").write_text(
        "Part 04 rough remint LANDED: hos_002_part04_rough_v19.mp4\n"
        "Watch WATCH_part04_v19.txt · PART04_V19_FLOW.md\n"
