#!/usr/bin/env python3
"""Plate UAT for 11b_wait_and_hunt try1 — continuous contact + text-edge + lamp/flask invent.
...
def judge(mp4: Path, out_dir: Path, try_id: str = "1") -> dict:
    ...
    for t in TIMES:
        ...
        still = out_dir / f"11b_wait_and_hunt_t{t:.1f}.jpg"
    ...
    contact = out_dir / f"11b_wait_and_hunt_contact_continuous_try{try_id}.jpg"
    contact_strip(frame_paths, contact, [f"t{t}" for t in TIMES])
    edge_contact = out_dir / f"11b_wait_and_hunt_edge_contact_try{try_id}.jpg"
