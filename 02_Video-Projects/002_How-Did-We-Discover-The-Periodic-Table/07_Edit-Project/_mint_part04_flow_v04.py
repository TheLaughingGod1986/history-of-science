#!/usr/bin/env python3
"""Part 04 Flow Veo 3.1 Fast partial remint v04 — scrub house silhouettes ×2.

Showrunner PART04_V04_SCRUB_HOUSE_SILHOUETTES.md (parent v03 sha 79dd7650…):
  - REMINT A: 02b_cards_sixty_three (~18–21 brown house silhouettes on books)
  - REMINT B: 09_risk_bet + 09b_risk_hold (~91–97 model-town through window)
  - KEEP: 05/06 night-sky ~38–44 · Explorer teal · Empty Chairs · glowing chair
  - Prefer real Veo Fast I2V. Create dies → STOP (no Ken Burns, no silhouette fills).
...
RAW = PROJ / "04_Generated-Clips/part04/raw/v04_fast"
META = PROJ / "07_Edit-Project/part04_mint_flow_v04_meta.json"
...
REJECT = (
    "HARD REJECT: brown house silhouettes; peaked-roof or chimney scrub masks; "
    "glowing yellow house props on books; house-icon tokens; miniature model-town "
    "toys; rooftop / town silhouettes through the window; glowing yellow street "
    "windows outside; model-town sill line; Ken Burns only; Orbit robot; photoreal; "
    "layout reset away from desk DNA."
)

DESK_PROP_LOCK = (
    "Leather-bound book stack may stay — book tops must be PLAIN leather OR blank "
    "cream cards ONLY. ZERO house silhouettes, ZERO peaked roofs, ZERO chimneys, "
    "ZERO brown scrub masks shaped like houses, ZERO glowing yellow house tokens."
)

WINDOW_LOCK = (
    "Through the lab window exterior: deep night sky with ONE moon and soft clouds "
    "/ faint stars ONLY. Completely EMPTY of architecture — no rooftops, no "
    "chimneys, no model-town, no village silhouette line on the sill, no glowing "
    "yellow street windows. Sky + moon + clouds only."
)
...
PROMPT_02B = (
    ...
    "Continuous card motion. Soft lamp. Night window with sky only. "
...
PROMPT_09 = (
    ...
    "and the ONE glowing vacant wooden chair (seat + back panel softly glowing — "
    "Empty Chairs / A BET). Continuous subtle camera drift + chair glow pulse. "
    f"{WINDOW_LOCK} "
    "Desk props stay (cream card stacks, vessels, balance). Silent. No people. "
    "No Explorer. KEEP chair glow. "
    + REJECT + " " + STYLE
)

PROMPT_09B = (
    "IMAGE-TO-VIDEO from the attached start frame. KEEP desk DNA and the glowing "
    "vacant chair as hero. Continuous subtle hold / lamp warmth vs cooler night "
    "window. "
    f"{WINDOW_LOCK} "
...
T2V_02B = (
    ...
    "cards settling in a dense stack, leather books with PLAIN clean tops (no "
    "houses), lab vessels. Continuous gentle card shuffle. Night sky window only. "
    "Silent. No people. " + STYLE + " " + REJECT + " " + DESK_PROP_LOCK
)

T2V_09 = (
    "ONE persistent 1869 chemist desk with ONE glowing vacant wooden chair "
    "(Empty Chairs / A BET). Continuous subtle drift. "
    f"{WINDOW_LOCK} Silent. No people. KEEP chair glow. "
    + STYLE + " " + REJECT
)

T2V_09B = (
    "ONE persistent 1869 chemist desk; glowing vacant chair holds frame. "
    "Continuous subtle camera hold. "
    f"{WINDOW_LOCK} Silent. KEEP chair glow. "
    + STYLE + " " + REJECT
)
...
DEFAULT_ONLY = ("02b_cards_sixty_three", "09_risk_bet", "09b_risk_hold")
ALLOWED_ONLY = set(DEFAULT_ONLY)

START_FRAMES = {
    "02b_primary": STILLS / "02b_books_clean_i2v.jpg",
    "02b_alt": STILLS / "02b_books_clean_alt.jpg",
    "02b_t1": STILLS / "02b_books_clean_t1.jpg",
    "02b_rough": STILLS / "02b_books_clean_from_rough.jpg",
    "09_primary": STILLS / "09_risk_sky_i2v.jpg",
    "09_alt": STILLS / "09_risk_sky_alt.jpg",
    "09_rough": STILLS / "09_risk_sky_from_rough.jpg",
    "09b_primary": STILLS / "09b_risk_sky_i2v.jpg",
    "09b_alt": STILLS / "09b_risk_sky_alt.jpg",
}
...
def resolve_start_frame(pid: str) -> Path:
    keys = {
        "02b_cards_sixty_three": ["02b_primary", "02b_alt", "02b_t1", "02b_rough"],
        "09_risk_bet": ["09_primary", "09_alt", "09_rough"],
        "09b_risk_hold": ["09b_primary", "09b_alt", "09_rough"],
    }.get(pid)
    if not keys:
...
def alt_starts_for(pid: str, start: Path) -> list[Path]:
    keys = {
        "02b_cards_sixty_three": ["02b_alt", "02b_t1", "02b_rough", "02b_primary"],
        "09_risk_bet": ["09_alt", "09_rough", "09_primary"],
        "09b_risk_hold": ["09b_alt", "09_rough", "09b_primary"],
    }.get(pid, [])
    out: list[Path] = []
    for key in keys:
        path = START_FRAMES[key]
...
def main() -> None:
    if _truthy("HOS_FLOW_SKIP_I2V"):
        print(
            "  WARN: unsetting HOS_FLOW_SKIP_I2V — v04 prefers clean I2V stills",
            flush=True,
        )
    ...
    meta: dict = {
        ...
        "raw": str(RAW),
        "continuity": "scrub_house_silhouettes_v04",
        "parent_v03_sha": "79dd7650cb582b548ae4c1c0aa98e4938ff8ec90ba654dea9c0b5c5574ee62e8",
        "only": sorted(only),
        "plates": [],
    }
    if META.exists():
    ...
    want = len(plates)
    print(f"OK part 04 Flow mint v04 finished ok={ok} want={want}", flush=True)
    if ok < want:
        sys.exit(2)
