#!/usr/bin/env python3
"""Part 03 Flow Veo 3.1 Fast partial remint v07 — plate 03_method_pamphlet ONLY.

Showrunner PART03_V07_EMPTY_CHAIR_SYNC_AND_SWIRL_MARKS.md (parent v06 sha a1285f9a…):
  - REMINT REQUIRED: 03_method_pamphlet under ATOMIC WEIGHTS
  - Props: KEEP desk marks (H 1 / O 16 / C 12) + 2–3 mid-air sheets H 1 · C 12 · O 16
  - MOST flying sheets blank. No equations/tables. Skip 04 by default.
  - Prefer I2V. Veo Fast. Create dies → STOP (no Ken Burns).
  - Auth: benoats@googlemail.com on /u/1/.
  - Do not remint empty chair / P01 / P02. No P04.
"""
...
RAW = PROJ / "04_Generated-Clips/part03/raw/v07_fast"
RAW_V02 = PROJ / "04_Generated-Clips/part03/raw/v02_fast"
META = PROJ / "07_Edit-Project/part03_mint_flow_v07_meta.json"
HARVEST = Path(__file__).resolve().parent / "_harvest_newest_gallery_v01.py"
STILLS = PROJ / "04_Generated-Clips/part03/refs/v07_stills"
STILLS_V06 = PROJ / "04_Generated-Clips/part03/refs/v06_stills"
...
V07_PROMPT_03 = (
    ...
    "Continuous motion: pamphlet swirl mid-air AND/OR desk hero with upright cream "
    "sheet + tidy stack. PROPS LOCK: KEEP desk marks already on the start frame "
    "(H 1, O 16, C 12 on the upright hero). ALSO keep sparse phone-readable "
    "dark-ink marks on ONLY 2–3 mid-air cream sheets: H 1, C 12, O 16 "
    "(element + number only). Huge high-contrast charcoal on cream. MOST flying "
    "sheets stay completely BLANK. NOT equations. NOT full tables. NOT paragraphs. "
    "NOT a wall of text. Continuous motion the whole clip. Opaque props. Silent. "
    "No people / no Explorer. Wonder not horror. "
    "HARD REJECT: every sheet covered; tiny chicken-scratch; formula walls; "
    "desk marks wiped; room swap / new architecture; green theatre chairs; "
    "microphone; blackboard/chandelier/fresco; Orbit; vessel fire; Ken Burns only."
)
...
ALLOWED_ONLY = {"03_method_pamphlet"}

START_FRAMES = {
    "03_desk_midair": STILLS / "03_desk_keep_plus_midair_t3.jpg",
    "03_desk_midair_t2": STILLS / "03_desk_keep_plus_midair_t2.jpg",
    "03_swirl_midair": STILLS / "03_swirl_plus_midair_t4.jpg",
    "03_fail_swirl": STILLS / "03_FAIL_swirl_v07_midair_hco.jpg",
    "03_desk_keep": STILLS / "03_FAIL_desk_marked_keep_v07.jpg",
    "03_fail_desk_v06": STILLS_V06 / "03_FAIL_desk_marked_i2v.jpg",
}
...
def t2v_prompt(pid: str) -> str:
    return (
        ...
        "tall arched windows, wooden benches. Continuous swirl of cream pamphlets "
        "mid-air down the aisle, settling toward a desk with one upright cream sheet. "
        "KEEP desk marks H 1 / O 16 / C 12 on the upright hero. On ONLY 2–3 mid-air "
        "sheets, sparse huge phone-readable dark-ink marks: H 1, C 12, O 16. "
        "MOST sheets blank. No equations, no tables, no wall of text. "
...
def main() -> None:
    ...
    if only != {"03_method_pamphlet"}:
        raise SystemExit(
            f"STOP: v07 remints ONLY 03_method_pamphlet; got {sorted(only)}"
        )
    ...
    meta: dict = {
        ...
        "raw": str(RAW),
        "continuity": "atomic_weights_swirl_midair_hco_v07",
        "source_keep_v02": str(RAW_V02),
        "parent_v06_sha_prefix": "a1285f9a",
        "only": sorted(only),
        "keep_explorer_v05": True,
        "keep_empty_chair_v04": True,
        "skip_04_default": True,
        "plates": [],
    }
    if META.exists():
        try:
    ...
    with sync_playwright() as p:
        ...
        try:
            for i, plate in enumerate(plates):
                ...
                force = os.environ.get("HOS_FLOW_FORCE_REMINT", "").strip().lower() in {
                    "1", "true", "yes",
                }
    ...
    want = len(only)
    print(f"OK part 03 Flow mint v07 finished ok={ok} want={want}", flush=True)
    if ok < want:
        sys.exit(2)
