#!/usr/bin/env python3
"""Part 05 plate 01b — closer wall, empty seats waiting. Cycle 3.

01 KEEP EWS classroom. 02 KEEP wall-pin hands. This plate: CLOSER wall,
empty card-cells waiting, no hands, no desk trenches, no lamp.
...
PROMPT_01B = (
    "TEXT-TO-VIDEO. Plate 01b_empty_seats_waiting try1 — empty seats waiting. "
    "VO beat: the guests have not arrived yet; vacant seats wait on the chart. "
    "ONE continuous 8s take, stylised 3D cartoon 1869 classroom. "
    "HERO ACTION (must be readable muted): CLOSER shot of a wooden WALL CHART of cream cards. "
    "Three empty wooden cells in the grid wait — solid wood backs, NOT holes, NOT trenches, "
    "NOT fire. Soft gold rim-light on those empty cells only. No hands. No people. "
    "Camera slow push on the empty cells. Distinct from a wide classroom with school desks. "
    "Distinct from a close-up of hands pinning a card. "
    + CLEAN
)

JOBS = [
    {
        "plate": "01b_empty_seats_waiting",
        "which": "closer_empty_cells",
        "prompt": PROMPT_01B,
        "out": QA_DIR / "submit_01b_empty_seats_waiting_try1.json",
    },
]
...
def submit_one(page, job: dict) -> dict:
    ...
    out = {
        ...
        "paint": "banned",
        "assemble": False,
        "note": "closer empty cells; no hands (02 has pin); no EWS desks (01 has those)",
    }
...
def main() -> None:
    with sync_playwright() as p:
        ...
        for job in JOBS:
            ...
            time.sleep(2)
        print(json.dumps({"submitted": urls, "next": "harvest 01b → plate UAT"}, indent=2), flush=True)
