#!/usr/bin/env python3
"""Dedicated 09b_risk_hold try10C submit on live Mini CDP.
...
LOCK = REF / "09b_risk_hold_start_v20_try10C_nolamp_bounce.jpg"
...
OUT = PROJ / "07_Edit-Project/_qa_part04_v20_flow/submit_09b_risk_hold_try10C.json"
...
def main() -> None:
    ...
    if not LOCK.exists():
        raise SystemExit(f"ABORT: missing try10C lock {LOCK}")
    s = sha(START)
    if s == FORBIDDEN_FLAT:
    ...
    if s != sha(LOCK):
        raise SystemExit("ABORT: active start != try10C lock")
    # try10C may share bytes with try10B cropL931 — that is intentional (same nocup DNA)
    for ban in (
        "molten bead",
        "shade cup",
        "OFF-FRAME",
    ...
    with sync_playwright() as p:
        ...
        tmp = Path("/tmp/hos_v20_09b_try10C_stub.mp4")
        if tmp.exists():
            tmp.unlink()
        info: dict = {}
        try:
        ...
        out = {
            ...
            "start_sha256": s,
            "start_dna": (
                "try10C nolamp bounce: try9C cardless moon empty-chair cropL931 — "
                "NO desk-lamp prop; warm OFF-SCREEN/OFF-FRAME left wall bounce + moonlight; "
                "bible 8ddbf99; prompt bans inventing lamp/shade/cup/bead; "
                "NOT try10A; NOT try8 KEEP; NOT flat; paint banned"
            ),
            "model": MODEL,
