"""Part 04 Flow Veo 3.1 Fast remint v11 — Ben FAIL (glasses / clean lamp / written cards).

Parent FAIL: hos_002_part04_rough_v10.mp4 sha eab7f8ec…
...
Remint: 06_explorer_leaves_gap, 05b_families_settle, 07_eka_placeholders,
07b_eka_names_rotate, 09_risk_bet, 09b_risk_hold.
KEEP: 02b desk-only if clean; P01–P03 frozen. Flow: benoats@googlemail.com on /u/1/.
...
RAW = PROJ / "04_Generated-Clips/part04/raw/v11_fast"
REJECTED = RAW / "_rejected"
QA_STILLS = PROJ / "07_Edit-Project/_qa_part04_v11_plate"
META = PROJ / "07_Edit-Project/part04_mint_flow_v11_meta.json"
...
MAX_CREATES = int(os.environ.get("HOS_V11_MAX_CREATES", "2"))
CREATE_OFFSET = int(os.environ.get("HOS_V11_CREATE_OFFSET", "0"))

START_FRAMES = {
    "06_explorer_leaves_gap": PROJ / "04_Generated-Clips/part04/refs/v11_start_frames/06_explorer_leaves_gap_start_v11.jpg",
    "05b_families_settle": PROJ / "04_Generated-Clips/part04/refs/v11_start_frames/05b_families_settle_start_v11.jpg",
    "07_eka_placeholders": PROJ / "04_Generated-Clips/part04/refs/v11_start_frames/07_eka_placeholders_start_v11.jpg",
    "07b_eka_names_rotate": PROJ / "04_Generated-Clips/part04/refs/v11_start_frames/07b_eka_names_rotate_start_v11.jpg",
    "09_risk_bet": PROJ / "04_Generated-Clips/part04/refs/v11_start_frames/09_risk_bet_start_v11.jpg",
    "09b_risk_hold": PROJ / "04_Generated-Clips/part04/refs/v11_start_frames/09b_risk_hold_start_v11.jpg",
}
START_FRAME_06 = START_FRAMES["06_explorer_leaves_gap"]
START_FRAME_PREF = START_FRAME_06
...
PARENT_V10 = PROJ / "09_Final-Export/hos_002_part04_rough_v10.mp4"
PARENT_V10_SHA = "eab7f8ec4d01ae21352e882dd50f8b45bab13d6305c2ac21f6fb73682e47ec8f"
...
PROMPTS = {
    "06_explorer_leaves_gap": (
        ...
        "History of Science DNA character sheet — FULL thick messy wavy chestnut hair "
        "covering the entire crown, fair warm-tan skin, dark teal coat, ROUND gold/"
    ...
    "05b_families_settle": (
        ...
        "no sparks, no candle flames. Continuous settle + gentle camera. Fully indoor "
        "wood/bookcase — NO night moon window, NO navy sky. Empty-seat glow OK. "
    ...
    "07_eka_placeholders": (
        ...
        "glowing empty card-slots / vacant seats among WRITTEN cream element cards "
        "(readable Eka-/H/C/O marks on neighboring cards — NEVER blank stacks). Soft "
        "warm desk-lamp glow ONLY — no fire spit, no candle. Fully indoor wood/bookcase "
        "— NO moon window, NO navy sky. Continuous soft pulse on empty seats. "
        f"{LAMP_CLEAN_LOCK} {WRITTEN_CARDS_LOCK} {WINDOW_LOCK} {DESK_PROP_LOCK} "
        "Silent. No people. "
        + REJECT + " " + STYLE
    ),
    "07b_eka_names_rotate": (
        "Image-to-video from the attached start frame. Same 1869 desk DNA. Soft pulse "
        "rotates across glowing vacant seats; surrounding cards show readable Eka- / "
        "element ink marks (NEVER blank cream faces). Soft warm desk-lamp glow ONLY — "
        "no fire spit, sparks, or candles. Fully indoor wood/bookcase — NO moon window. "
        f"{LAMP_CLEAN_LOCK} {WRITTEN_CARDS_LOCK} {WINDOW_LOCK} {DESK_PROP_LOCK} "
        "Silent. No people. "
        + REJECT + " " + STYLE
    ),
    "09_risk_bet": (
        ...
        "desk-lamp glow ONLY — no fire spit, no sparks, no candle flames. KEEP Empty "
        "Chairs chair glow. Continuous subtle camera drift. Fully indoor wood/bookcase. "
        f"{LAMP_CLEAN_LOCK} {WRITTEN_CARDS_LOCK} {WINDOW_LOCK} {DESK_PROP_LOCK} "
        "Silent. No people. No Explorer. "
        + REJECT + " " + STYLE
    ),
    "09b_risk_hold": (
        "Image-to-video from the attached start frame. Same 1869 desk DNA. Glowing vacant "
        "chair holds frame; WRITTEN cream element cards (readable symbols) stay on desk — "
        "NEVER blank. Soft warm desk-lamp glow ONLY — no fire spit/candle. Continuous "
        "subtle hold. Fully indoor wood/bookcase. KEEP chair glow. "
        f"{LAMP_CLEAN_LOCK} {WRITTEN_CARDS_LOCK} {WINDOW_LOCK} {DESK_PROP_LOCK} "
        "Silent. No people. "
        + REJECT + " " + STYLE
    ),
...
def dest_for(plate_id: str) -> Path:
    return RAW / f"{plate_id}_v11.mp4"
...
def resolve_start_frame(plate_id: str | None = None) -> Path:
    ...
    for cand in (START_FRAME_06, START_FRAME_PREF):
        if cand.exists() and cand.stat().st_size > 20_000:
            return cand
    raise SystemExit(
        f"STOP: missing profile/back start frame ({START_FRAME_06} or {START_FRAME_PREF})"
    )
...
def main() -> None:
    ...
    if not _truthy("HOS_FLOW_T2V_ONLY") and not _truthy("HOS_FLOW_I2V_FIRST"):
        ...
        print("  v11 default: I2V Ben-fail locks (glasses/lamp/cards)", flush=True)
    ...
    meta: dict = {
        ...
        "raw": str(RAW),
        "continuity": "v11_ben_fail_glasses_lamp_cards",
        "parent_v10_sha": PARENT_V10_SHA,
