#!/usr/bin/env python3
"""Part 04 Flow Veo 3.1 Fast remint v05 — 02b ONLY (scrub unblended brown panel).

Showrunner PART04_V05_SCRUB_BROWN_PANEL.md (parent v04 sha fda60b04…):
  - REMINT: 02b_cards_sixty_three ONLY (~18–21 unblended brown scrub/panel)
  - KEEP: 09/09b late window · 05/06 night-sky · Explorer teal · Empty Chairs
  - FULL Veo 3.1 Fast. Create dies OR gallery fail → STOP.
  - NEVER hard fills / brown panels / flat sky rectangles (that is the v04 fail).
  - Prefer T2V (v01 02b DNA already bakes brown scrub + houses). I2V only from
    clean plate-02 DNA still. Auth: benoats@googlemail.com on /u/1/. P01–P03 FROZEN.
"""
...
RAW = PROJ / "04_Generated-Clips/part04/raw/v05_fast"
META = PROJ / "07_Edit-Project/part04_mint_flow_v05_meta.json"
...
REJECT = (
    "HARD REJECT: unblended brown scrub panels; flat rectangular leather overlays; "
    "hard rectangular fills of any colour; flat blue sky boxes / pasted sky "
    "rectangles cutting across desk or cards; brown house silhouettes; peaked-roof "
    "or chimney scrub masks; glowing yellow house props on books; house-icon "
    "tokens; miniature model-town toys; rooftop / town silhouettes through the "
    "window; glowing yellow street windows outside; model-town sill line; "
...
WINDOW_LOCK = (
    "Through the lab window: deep night sky with ONE moon and soft clouds / faint "
    "stars rendered IN-CAMERA through real window panes — NOT a pasted flat sky "
    "rectangle. Completely EMPTY of architecture — no rooftops, no chimneys, no "
    "model-town, no village silhouette on the sill."
)
...
T2V_02B = (
    ...
    "flasks, white mortar, night window with moon and soft clouds through real "
    "panes. Continuous gentle card shuffle and subtle camera drift. Silent. No "
    "people. No hands. No Explorer. "
    + STYLE + " " + REJECT + " " + DESK_PROP_LOCK + " " + WINDOW_LOCK
)
...
# Prefer plate-02 DNA (continuous desk, no brown panel). Avoid v01 02b grabs —
# those already bake house tokens + brown scrub into the start frame.
START_FRAMES = {
    "02b_primary": STILLS / "02b_desk_clean_from_02.jpg",
    "02b_alt": STILLS / "02b_desk_clean_alt.jpg",
    "02b_t1": STILLS / "02b_desk_clean_t1.jpg",
    "02b_i2v": STILLS / "02b_desk_clean_i2v.jpg",
}
...
def resolve_start_frame(pid: str) -> Path:
    keys = {
        "02b_cards_sixty_three": ["02b_primary", "02b_alt", "02b_t1", "02b_i2v"],
    }.get(pid)
    if not keys:
...
def alt_starts_for(pid: str, start: Path) -> list[Path]:
    keys = {
        "02b_cards_sixty_three": ["02b_alt", "02b_t1", "02b_i2v", "02b_primary"],
    }.get(pid, [])
    out: list[Path] = []
    for key in keys:
        path = START_FRAMES[key]
...
def main() -> None:
    # v05 default = T2V (avoids contaminated I2V stills). Set HOS_FLOW_I2V_FIRST=1
    # to try plate-02 DNA I2V first instead.
    if not _truthy("HOS_FLOW_I2V_FIRST") and not _truthy("HOS_FLOW_T2V_ONLY"):
        os.environ["HOS_FLOW_T2V_ONLY"] = "1"
        print(
            "  v05 default: HOS_FLOW_T2V_ONLY=1 (no hard-fill; avoid baked brown scrub)",
            flush=True,
        )
    ...
    meta: dict = {
        ...
        "raw": str(RAW),
        "continuity": "scrub_brown_panel_v05_veo_no_hard_fill",
        "parent_v04_sha": "fda60b04bd3785b1f01b61c5c76b786a841d21152df29ed398ce5b5e4490554d",
        "only": sorted(only),
        "plates": [],
    }
    if META.exists():
    ...
    with sync_playwright() as p:
        ...
        try:
            for i, plate in enumerate(plates):
                pid = plate["id"]
                dest = dest_for(pid)
                start = None if _truthy("HOS_FLOW_T2V_ONLY") else resolve_start_frame(pid)
