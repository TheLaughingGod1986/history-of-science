#!/usr/bin/env python3
"""Assemble HOS 002 Part 01 rough — unique Flow plates + locked VO.

No freeze-pad. No loop. No slow-mo. Picture must cover VO (12×~8s with
0.4s xfade ≈ 91.6s ≥ 85.7s VO). Trim picture to VO with -shortest.
"""
...
OUT = PROJ / "09_Final-Export/hos_002_part01_rough_v01.mp4"
WORK = PROJ / "07_Edit-Project/_work_part01_v01"
CLIP_USE = 8.0
XFADE = 0.4
...
def main() -> None:
    ...
    if pic_dur + 0.05 < vo_dur:
        raise SystemExit(
            f"STOP: picture {pic_dur:.2f}s < VO {vo_dur:.2f}s — mint more unique plates, do not freeze-pad"
        )
    print(
        f"assemble plates={len(clips)} clip_use={CLIP_USE} xfade={XFADE} "
        f"pic≈{pic_dur:.2f}s vo={vo_dur:.2f}s → {OUT.name}",
        flush=True,
    )
    ...
    # Trim picture to VO length (extra picture allowed; never pad picture)
    parts.append(
        f"[{vprev}]trim=0:{vo_dur:.3f},setpts=PTS-STARTPTS[vout]"
    )
    filt = ";".join(parts)
    cmd = [
        "ffmpeg",
