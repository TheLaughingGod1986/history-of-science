#!/usr/bin/env python3
"""Assemble HOS 002 Part 04 rough v02 — scrub model-town window.

Parent: hos_002_part04_rough_v01.mp4 sha 543fd388…
  - REMINT ONLY 05_columns_families + 06_explorer_leaves_gap (window houses)
  - KEEP all other v01 plates, VO, bed, side labels, PLATE_ORDER
...
LABEL_DIR = PROJ / "04_Generated-Clips/part04/refs/v02_side_labels"
VO = PROJ / "02_Voiceover/part04_empty_chairs_v01.wav"
BED = PROJ / "05_Music/hos_002_part01_curious_workshop_v02_norm.wav"
OUT = PROJ / "09_Final-Export/hos_002_part04_rough_v02.mp4"
KEEP_V01 = PROJ / "09_Final-Export/hos_002_part04_rough_v01.mp4"
KEEP_V01_SHA = "543fd388f1bc1f1502d1eb45faf28e494658ec2ccc4749cce76bc5167b7a6a03"
...
REMINT_V02_IDS = {"05_columns_families", "06_explorer_leaves_gap"}
...
def resolve_clip(pid: str) -> tuple[Path, str]:
    if pid in REMINT_V02_IDS:
        v02 = RAW_V02 / f"{pid}_v02.mp4"
        if v02.exists() and v02.stat().st_size >= 400_000:
            return v02, "v02_scrub"
        raise SystemExit(f"missing reminted v02 plate {v02}")
...
def main() -> None:
    ...
    if not KEEP_V01.exists():
        raise SystemExit(f"missing parent v01 {KEEP_V01}")
    v01_sha = sha256(KEEP_V01)
    if v01_sha != KEEP_V01_SHA:
        raise SystemExit(
            f"STOP: v01 parent sha mismatch want {KEEP_V01_SHA} got {v01_sha}"
        )
    print(f"KEEP v01 parent sha OK {v01_sha}", flush=True)
    ...
    dur = probe(OUT)
    reminted = [pid for pid, src in zip(ordered_ids, sources) if src == "v02_scrub"]
    print(
        f"LANDED {OUT}\n"
        f"  path={OUT}\n"
        f"  bytes={bytes_n}\n"
    ...
    watch = ICLOUD / "WATCH_part04_v02.txt"
    watch.write_text(
        "WATCH THIS FILE ONLY (Part 04 rough v02 — scrub model-town window):\n"
        f"  {OUT.name}\n\n"
        "CoS / Picture→UAT gates for this cut:\n"
        "- Parent v01 FAIL ~40–45s: glowing yellow-window model-town outside lab window.\n"
        "- Reminted: 05_columns_families + 06_explorer_leaves_gap only.\n"
        "- Window = night sky + moon/stars ONLY (no miniature houses / yellow windows).\n"
        "- KEEP: Explorer teal trenchcoat · VO/labels · desk DNA · same PLATE_ORDER.\n"
        "- Real Veo Fast remints. Continuous motion. No Ken Burns.\n"
    ...
    (ICLOUD / "ZZ_OPEN_PART04_V02_ONLY.txt").write_text(
        "Part 04 current cut = hos_002_part04_rough_v02.mp4\n"
        "Part 01 v14 + Part 02 v06 + Part 03 v09 are LOCKED keepers.\n"
        "v01 was UAT FAIL (model-town window) — watch v02 only.\n"
    )
    (ICLOUD / "PART04_NEXT.txt").write_text(
        "Part 04 rough remint LANDED: hos_002_part04_rough_v02.mp4\n"
        "Watch WATCH_part04_v02.txt\n"
