#!/usr/bin/env python3
"""Assemble HOS 002 Part 04 rough v17 — PUBLISH THE GAPS sharp remint.

Parent KEEP: hos_002_part04_rough_v16.mp4
  sha256 7e083fc440879209bbc69459fbaf21dc29dbfc1f77b370ec92ee931bec82cb3c
Single blocker: LATE SHOTS SHARP — PUBLISH ~120–126 still ghost doubles.

Remint ONLY: 11_publish_gaps, 11b_wait_and_hunt
KEEP: 10_family_before_weight (v16), Explorer 06, 09/09b CLEAN LIGHT, written cards ~40.

Do NOT restore ghosty v01 DNA. Scores → CoS. Do not declare PASS. Do not ping Ben.
"""
...
OUT = PROJ / "09_Final-Export/hos_002_part04_rough_v17.mp4"
...
KEEP_V16_IDS = {
    "10_family_before_weight",  # FAMILY FIRST KEEP (cleared)
}
KEEP_V15_IDS = {
    "06_explorer_leaves_gap",  # Explorer crown KEEP
    "09_risk_bet",
...
def resolve_clip(pid: str) -> tuple[Path, str]:
    if pid in REMINT_V17_IDS:
        v17 = RAW_V17 / f"{pid}_v17.mp4"
        if v17.exists() and v17.stat().st_size >= 400_000:
            return v17, "v17_publish_sharp_remint"
        raise SystemExit(f"missing reminted v17 plate {v17}")
...
def main() -> None:
    ...
    dur = probe(OUT)
    reminted = [pid for pid, src in zip(ordered_ids, sources) if src == "v17_publish_sharp_remint"]
    plate06 = RAW_V15 / "06_explorer_leaves_gap_v15.mp4"
    plate06_sha = sha256(plate06) if plate06.exists() else None
    plate10 = RAW_V16 / "10_family_before_weight_v16.mp4"
    ...
    methods = {
        pid: "fresh Flow/Veo single-exposure remint (NOT v01 restore, NOT temporal-median)"
        for pid in reminted
    }
    meta_path = PROJ / "07_Edit-Project/part04_rough_v17_land_meta.json"
        ...
        json.dumps(
            {
                ...
                "plates": {
                    pid: {
                        "path": str((RAW_V17 / f"{pid}_v17.mp4").relative_to(PROJ)),
                        "sha256": plate_sha[pid],
                        "bytes": (RAW_V17 / f"{pid}_v17.mp4").stat().st_size,
                    }
                    for pid in reminted
                },
                "methods": methods,
                "kept_from_v16": sorted(KEEP_V16_IDS),
                "kept_from_v15": sorted(KEEP_V15_IDS),
                "uat_blocker": "LATE SHOTS SHARP — PUBLISH THE GAPS ghost doubles ~120–126",
                "fail_times": {"publish_ghost": 121, "grid_ghost": 125},
                "timeline": {
                ...
                "doc": "PART04_V17_PUBLISH_SHARP.md",
                "status": (
    ...
    watch = ICLOUD / "WATCH_part04_v17.txt"
    watch.write_text(
        "WATCH THIS FILE ONLY (Part 04 rough v17 — PUBLISH THE GAPS sharp remint):\n"
        f"  {OUT.name}\n\n"
        "CoS / Picture→UAT gates for this cut:\n"
        "- Parent KEEP v16 sha 7e083fc440879209… — blocker LATE SHOTS SHARP on PUBLISH only.\n"
        "- Reminted ONLY 11 / 11b via fresh Flow/Veo (NOT ghosty v01 restore).\n"
        "- KEEP: plate 10 FAMILY FIRST, Explorer 06, 09/09b CLEAN LIGHT, written cards ~40.\n"
        "- Spot ~110 FAMILY FIRST KEEP: H/C/N/O sharp — do not regress.\n"
        "- Spot ~121 / ~125 PUBLISH THE GAPS: one solid lamp, one solid flask set, one solid "
        "grid — NO ghost doubles, NO left-half pixel mush.\n"
        "- Spot ~102–105 CLEAN LIGHT KEEP: soft panel, no lamp lava.\n"
    ...
    (PROJ / "07_Edit-Project/WATCH_part04_v17.txt").write_text(watch.read_text())
    (ICLOUD / "ZZ_OPEN_PART04_V17_ONLY.txt").write_text(
        "Part 04 current cut = hos_002_part04_rough_v17.mp4\n"
        "Part 01–03 LOCKED keepers.\n"
        "v16 FAIL (publish ghosts @ ~121–125) → v17 publish-sharp remint — watch v17 only.\n"
    )
    (ICLOUD / "PART04_NEXT.txt").write_text(
        "Part 04 rough remint LANDED: hos_002_part04_rough_v17.mp4\n"
        "Watch WATCH_part04_v17.txt · PART04_V17_PUBLISH_SHARP.md\n"
