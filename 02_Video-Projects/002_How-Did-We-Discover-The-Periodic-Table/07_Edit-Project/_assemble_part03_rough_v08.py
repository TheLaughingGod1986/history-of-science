#!/usr/bin/env python3
"""Assemble HOS 002 Part 03 rough v08 — scrub model-town / blue-yellow blocks.

Parent: hos_002_part03_rough_v07.mp4 sha f39d794e…
  - REMINT 08_property_waves (soft colour light pulses on glowing blank mass-line)
  - REMINT 10_city_plan_lots (postcard-scale flat plan on existing desk ONLY)
  - KEEP EMPTY CHAIR sync + chair art (v04) — no remint
  ...
  - Same PLATE_ORDER as v07. No P01/P02 remint. No P04. Scores → CoS only.
"""
...
LABEL_DIR = PROJ / "04_Generated-Clips/part03/refs/v08_side_labels"
VO = PROJ / "02_Voiceover/part03_ruler_for_atoms_v01.wav"
BED = PROJ / "05_Music/hos_002_part01_curious_workshop_v02_norm.wav"
OUT = PROJ / "09_Final-Export/hos_002_part03_rough_v08.mp4"
KEEP_V07 = PROJ / "09_Final-Export/hos_002_part03_rough_v07.mp4"
KEEP_V07_SHA_PREFIX = "f39d794e"
ICLOUD = Path.home() / "Library/Mobile Documents/com~apple~CloudDocs/HOS UAT"
XFADE = 0.35
BED_VOL = 0.38

# Same mild early compress as v07 so plate 09 starts ~52.0 under spoken chair.
EARLY_USE = 7.78  # plates before empty chair
CLIP_USE = 8.0

REMINT_V08_IDS = {"08_property_waves", "10_city_plan_lots"}
REMINT_V07_IDS = {"03_method_pamphlet"}
EXPLORER_V05_IDS = {"05_explorer_ruler"}
CHAIR_V04_IDS = {"09_empty_chair_claim"}

# Same order as v07 (chair early, then 08 → 10 → 11 → 12).
PLATE_ORDER = [
...
# Labels retimed as v07 (EMPTY CHAIR + A PREDICTION required).
SIDE_LABELS = [
...
def resolve_clip(pid: str) -> tuple[Path, str]:
    ...
    if pid in REMINT_V08_IDS:
        v08 = RAW_V08 / f"{pid}_v08.mp4"
        if v08.exists() and v08.stat().st_size >= 400_000:
            return v08, "v08"
        raise SystemExit(f"missing reminted v08 plate {v08}")
...
def main() -> None:
    if not KEEP_V07.exists():
        raise SystemExit(f"missing KEEP parent {KEEP_V07}")
    keep_sha = sha256(KEEP_V07)
    if not keep_sha.startswith(KEEP_V07_SHA_PREFIX):
        raise SystemExit(
            f"STOP: KEEP v07 sha mismatch want prefix {KEEP_V07_SHA_PREFIX}… got {keep_sha}"
        )
    print(f"KEEP v07 parent sha OK {keep_sha}", flush=True)
    ...
    for old in ICLOUD.glob("hos_002_part03_rough_v*.mp4"):
        ...
        if old.exists() and not superseded.exists():
            ...
            old.unlink()
    watch = ICLOUD / "WATCH_part03_v08.txt"
    watch.write_text(
        "WATCH THIS FILE ONLY (Part 03 rough v08 — scrub model-town / blue-yellow blocks):\n"
        f"  {OUT.name}\n\n"
        "CoS / Picture→UAT gates for this cut:\n"
        "- Parent v07 sha f39d794e…; reminted REQUIRED 08_property_waves + 10_city_plan_lots only.\n"
        "- ~61s property beat: soft colour light pulses along glowing blank mass-line — "
        "NOT toy blocks / prismatic brick piles / model-town.\n"
        "- ~68s city-plan beat: postcard-scale flat plan on existing desk/bench ONLY — "
        "HARD REJECT central-table model-town sprawl / blue-yellow block toys.\n"
        "- EMPTY CHAIR picture ~52.0–60.0 under spoken chair; label EMPTY CHAIR 52.4–58.5 KEEP.\n"
        "- KEEP locked: 03 swirl H/C/O v07, 05 Explorer teal trenchcoat, Karlsruhe blank (02), "
        "chair art v04, VO part03_ruler_for_atoms_v01, same PLATE_ORDER as v07.\n"
        ...
        f"keep_v07_sha={keep_sha}\n"
        f"chair_picture={chair_start:.3f}-{chair_end:.3f}\n"
        f"waves≈{waves_start:.3f} city≈{city_start:.3f}\n"
        f"sha256={digest}\nbytes={bytes_n}\nduration={dur:.3f}\n"
    )
    (ICLOUD / "ZZ_OPEN_PART03_V08_ONLY.txt").write_text(
        "Part 03 current cut = hos_002_part03_rough_v08.mp4\n"
        "Part 01 v14 + Part 02 v06 are PASS locked keepers.\n"
    )
    (ICLOUD / "PART03_NEXT.txt").write_text(
        "Part 03 rough LANDED: hos_002_part03_rough_v08.mp4\n"
        "Watch WATCH_part03_v08.txt\n"
        "v08: scrub model-town / blue-yellow blocks on 08+10; chair+swirl+Explorer KEPT.\n"
