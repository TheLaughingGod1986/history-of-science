"""Part 04 Flow Veo 3.1 Fast remint v12 — CLEAN LIGHT + WRITTEN CARDS (chair fire / sparks / blank cards).

Parent FAIL: hos_002_part04_rough_v12.mp4 sha 96a0f41e…
Ben blockers HARD: (1) Explorer round glasses always visible (2) lamp warm glow only —
no fire spit/sparks/candle (3) element cards show readable H/C/O/Eka marks — never blank.

Remint: 04,05,05b,07,08,08b,09,09b,10.
KEEP: 06_explorer_leaves_gap CLEARED; 02b if clean; 07b; P01–P03 frozen. Flow: benoats@googlemail.com on /u/1/.
...
RAW = PROJ / "04_Generated-Clips/part04/raw/v12_fast"
REJECTED = RAW / "_rejected"
QA_STILLS = PROJ / "07_Edit-Project/_qa_part04_v12_plate"
META = PROJ / "07_Edit-Project/part04_mint_flow_v12_meta.json"
...
MAX_CREATES = int(os.environ.get("HOS_V12_MAX_CREATES", "2"))
CREATE_OFFSET = int(os.environ.get("HOS_V12_CREATE_OFFSET", "0"))

START_FRAMES = {
    "04_sort_atomic_weight": PROJ / "04_Generated-Clips/part04/refs/v12_start_frames/04_sort_atomic_weight_start_v12.jpg",
    "05_columns_families": PROJ / "04_Generated-Clips/part04/refs/v12_start_frames/05_columns_families_start_v12.jpg",
    "05b_families_settle": PROJ / "04_Generated-Clips/part04/refs/v12_start_frames/05b_families_settle_start_v12.jpg",
    "07_eka_placeholders": PROJ / "04_Generated-Clips/part04/refs/v12_start_frames/07_eka_placeholders_start_v12.jpg",
    "08_prediction_navigation": PROJ / "04_Generated-Clips/part04/refs/v12_start_frames/08_prediction_navigation_start_v12.jpg",
    "08b_navigation_walk": PROJ / "04_Generated-Clips/part04/refs/v12_start_frames/08b_navigation_walk_start_v12.jpg",
    "09_risk_bet": PROJ / "04_Generated-Clips/part04/refs/v12_start_frames/09_risk_bet_start_v12.jpg",
    "09b_risk_hold": PROJ / "04_Generated-Clips/part04/refs/v12_start_frames/09b_risk_hold_start_v12.jpg",
    "10_family_before_weight": PROJ / "04_Generated-Clips/part04/refs/v12_start_frames/10_family_before_weight_start_v12.jpg",
}
START_FRAME_PREF = START_FRAMES["05b_families_settle"]
DNA_GERMS = PROJ / "04_Generated-Clips/part04/refs/v11_dna/germs_lock.jpg"
DNA_P03 = PROJ / "04_Generated-Clips/part04/refs/v11_dna/p03_keep.jpg"
DNA_SHEET = PROJ / "04_Generated-Clips/part04/refs/v11_dna/character_sheet.jpg"
DNA_OK_PROFILE = PROJ / "04_Generated-Clips/part04/refs/v11_dna/ok_profile_49.jpg"

PARENT_V11 = PROJ / "09_Final-Export/hos_002_part04_rough_v11.mp4"
PARENT_V11_SHA = "96a0f41e53987e090e5eee6af29d3abda8de3c30b326584193c10847d1ebd739"
...
GLASSES_LOCK = (
    "CRITICAL EVERY FRAME Explorer is visible: ROUND gold or thin dark WIRE-RIM "
    "GLASSES on his face — same as History of Science character sheet / Germs / P03. "
    "Glasses must stay readable in PROFILE and over-shoulder (rim + lens catch light). "
    "HARD REJECT glasses-free cousin, no frames, solid white eyes without rims."
)
...
WRITTEN_CARDS_LOCK = (
    "WRITTEN CARDS LOCK: every visible element/period card shows readable hand-ink "
    "letters/symbols/names — at least H, C, O and/or Eka- marks with small numbers. "
    "HARD REJECT blank white/cream card stacks, blank hero cards, empty card faces."
)
...
REJECT = (
    ...
    "academic blazer instead of teal trenchcoat; teaching/presenting-to-camera pose; face-on Explorer; ¾-front hero; hands-on-hips present-to-viewer; mid-clip flip from profile to face-on; Explorer without glasses; blank cream card stacks; blank hero cards; lamp spitting fire; sparks under bulb; candle flames on desk."
)
...
EXPLORER_GARNISH_LOCK = (
    ...
    "Explorer is a small accent, NEVER a medium portrait. "
    "POSE: PROFILE facing the cards OR back / over-shoulder — never face-on. "
    "Full messy chestnut crown stays readable. "
    "HARD REJECT face-to-camera, ¾-front hero, hands-on-hips present, waist-up hero, "
    "close-up face, or Explorer filling the middle of the frame."
)

PROMPTS = {
    "04_sort_atomic_weight": (
        "Image-to-video from the attached start frame. Same 1869 chemist desk DNA. "
        "WRITTEN cream element cards (readable ink H/C/O/N/Li marks — NEVER blank) "
        "slide and sort by atomic weight across the honey wood. Soft warm desk-lamp "
        "glow ONLY — no fire spit, sparks, candle flames, or smoke. Continuous sort "
        "motion. Fully indoor wood/bookcase — NO moon window, NO flat blue sky. "
        f"{LAMP_CLEAN_LOCK} {WRITTEN_CARDS_LOCK} {WINDOW_LOCK} {DESK_PROP_LOCK} "
        "Silent. No people. No Explorer. "
        + REJECT + " " + STYLE
    ),
    "05_columns_families": (
        "Image-to-video from the attached start frame. Same 1869 desk DNA. Columns of "
        "WRITTEN cream element cards (readable H/C/O/N/Li/Eka ink — NEVER blank stacks "
        "or blank flying cards) settle into family columns. Soft warm desk-lamp glow "
        "ONLY — no sparks under bulb, no candle flames. Continuous settle motion. "
        "Fully indoor wood/bookcase. "
        f"{LAMP_CLEAN_LOCK} {WRITTEN_CARDS_LOCK} {WINDOW_LOCK} {DESK_PROP_LOCK} "
        "Silent. No people. No Explorer. "
        + REJECT + " " + STYLE
    ),
    "05b_families_settle": (
        "Image-to-video from the attached start frame. Same 1869 desk DNA. Family columns "
        "of WRITTEN cream element cards (readable H/C/O/N/Li/Eka ink marks — NEVER blank) "
        "finish settling on the honey wood. Soft warm desk-lamp glow ONLY — no fire spit, "
        "no sparks under the bulb, no candle flames, no smoke. Continuous settle + gentle "
        "camera. Fully indoor wood/bookcase — NO night moon window, NO navy sky. "
        "Empty-seat soft glow OK. "
        f"{LAMP_CLEAN_LOCK} {WRITTEN_CARDS_LOCK} {WINDOW_LOCK} {DESK_PROP_LOCK} "
        "Silent. No people. No Explorer. "
        + REJECT + " " + STYLE
    ),
    "07_eka_placeholders": (
        ...
        "rectangles. Soft Empty-Chairs glow NOT fire. Fully indoor wood/bookcase. "
        "Continuous soft pulse on empty seats. "
        f"{LAMP_CLEAN_LOCK} {WRITTEN_CARDS_LOCK} {WINDOW_LOCK} {DESK_PROP_LOCK} "
        "Silent. No people. No Explorer. "
        + REJECT + " " + STYLE
    ),
    "08_prediction_navigation": (
        "Image-to-video from the attached start frame. Same 1869 desk DNA. WRITTEN cream "
        "element cards (readable H/C/O/Eka ink — NEVER blank) guide a prediction path "
        "across the desk. Soft warm desk-lamp glow ONLY — no chair fire, no sparks, no "
        "smoke. Soft Empty-Chairs chair glow OK (NOT fire). Fully indoor wood/bookcase "
        "— NO moon window, NO flat blue sky. Continuous subtle camera. "
        f"{LAMP_CLEAN_LOCK} {WRITTEN_CARDS_LOCK} {WINDOW_LOCK} {DESK_PROP_LOCK} "
        "Silent. No people. No Explorer. "
        + REJECT + " " + STYLE
    ),
    "08b_navigation_walk": (
        "Image-to-video from the attached start frame. Same 1869 desk DNA. Hero cream "
        "element card propped on a stack shows readable ink symbol (H/C/O/N/Eka — NEVER "
        "a blank white hero card). Neighbor cards also WRITTEN. Soft warm desk-lamp glow "
        "ONLY — no sparks, no candle-flame lamp artifacts, no smoke. Fully indoor "
        "wood/bookcase — NO moon window. Continuous subtle hold + gentle drift. "
        f"{LAMP_CLEAN_LOCK} {WRITTEN_CARDS_LOCK} {WINDOW_LOCK} {DESK_PROP_LOCK} "
        "Silent. No people. No Explorer. "
        + REJECT + " " + STYLE
    ),
    "09_risk_bet": (
        "Image-to-video from the attached start frame. Same 1869 desk DNA. Empty glowing "
        "chair / vacant seat holds the bet; cream element cards on the desk show readable "
        "H/C/O/Eka ink marks — NEVER blank white stacks or blank hero cards. Soft warm "
        "desk-lamp glow ONLY — no fire spit, no sparks, no candle flames, no leather "
        "chair on fire, no smoke wisps. KEEP soft Empty-Chairs chair glow (NOT fire). "
        "Continuous subtle camera drift. Fully indoor wood/bookcase. "
        f"{LAMP_CLEAN_LOCK} {WRITTEN_CARDS_LOCK} {WINDOW_LOCK} {DESK_PROP_LOCK} "
        "Silent. No people. No Explorer. "
        + REJECT + " " + STYLE
    ),
    "09b_risk_hold": (
        ...
        "vacant chair holds frame — soft Empty-Chairs glow NOT fire/flames in leather. "
        "WRITTEN cream element cards (readable symbols) stay on desk — NEVER blank. "
        "Soft warm desk-lamp glow ONLY — no fire spit/candle/smoke. Continuous subtle "
        "hold. Fully indoor wood/bookcase. "
        f"{LAMP_CLEAN_LOCK} {WRITTEN_CARDS_LOCK} {WINDOW_LOCK} {DESK_PROP_LOCK} "
        "Silent. No people. No Explorer. "
        + REJECT + " " + STYLE
    ),
    "10_family_before_weight": (
        "Image-to-video from the attached start frame. Same 1869 desk DNA. WRITTEN cream "
        "element cards (readable H/C/O/N/family marks — NEVER blank floating cards) "
        "hover and settle showing family-before-weight. Soft warm desk-lamp glow ONLY "
        "— no chair fire, no sparks, no smoke. Soft Empty-Chairs glow OK. Fully indoor "
        "wood/bookcase — NO moon window, NO flat blue sky. Continuous gentle float. "
        f"{LAMP_CLEAN_LOCK} {WRITTEN_CARDS_LOCK} {WINDOW_LOCK} {DESK_PROP_LOCK} "
        "Silent. No people. No Explorer. "
        + REJECT + " " + STYLE
    ),
...
def dest_for(plate_id: str) -> Path:
    return RAW / f"{plate_id}_v12.mp4"


def resolve_start_frame(plate_id: str | None = None) -> Path:
    """Ben-fail start: written cards + clean lamp (+ glasses on 06)."""
    if plate_id and plate_id in START_FRAMES:
        cand = START_FRAMES[plate_id]
        if cand.exists() and cand.stat().st_size > 20_000:
            return cand
        raise SystemExit(f"STOP: missing start frame for {plate_id}: {cand}")
    for cand in (START_FRAME_PREF, START_FRAME_PREF):
        if cand.exists() and cand.stat().st_size > 20_000:
            return cand
    raise SystemExit(
        f"STOP: missing profile/back start frame ({START_FRAME_PREF} or {START_FRAME_PREF})"
    )
...
def run_edit_download(dest: Path, project_url: str, edit_url: str = "") -> None:
    project_url = (project_url or "").split("?")[0].rstrip("/")
    env = dict(os.environ)
    env["ORBIT_FLOW_HOME"] = flow.FLOW_HOME
    env["ORBIT_FLOW_PROFILE"] = str(PROFILE)
    cmd = [sys.executable, "-u", str(EDIT_DL), "--project", project_url, "--dest", str(dest)]
    if edit_url:
        cmd += ["--edit", edit_url]
    print(f"  spawn edit-download: {' '.join(cmd)}", flush=True)
    subprocess.check_call(cmd, env=env)
...
def run_harvest(dest: Path, project_url: str, *, before_thumbs: int = -1) -> None:
    ...
    try:
        ...
        try:
            ...
            print(f"  force-download failed ({force_exc}); trying edit-download", flush=True)
            run_edit_download(dest, project_url)
...
def explorer_faceon_pose_suspect(still: Path) -> dict:
    """Reject face-on / ¾-front hero (v09 UAT fail ~51–52s).

    Profile KEEP: skin mass sits on ONE side of the teal garnish bbox (facing cards).
    Face-on FAIL: bilateral skin/eyes centered in the upper teal bbox, often wide stance.
    """
    im = Image.open(still).convert("RGB")
    w, h = im.size
    xs, ys = _teal_mask_pts(im)
    if len(xs) < 30:
        return {
            "still": str(still), "faceon": False, "reason": "little_teal",
            "eye_pairs": 0, "skin_frac": 0.0, "skin_cx_norm": None, "sym": None,
        }
    x0, x1 = min(xs), max(xs)
    y0, y1 = min(ys), max(ys)
    bw = max(1, x1 - x0)
    ...
    for y in range(band_y0, band_y1, 1):
        for x in range(x0, x1 + 1, 1):
            ...
            lum = (r + g + b) / 3.0
            # warm fair skin
            if 95 < lum < 225 and r > 105 and g > 75 and b > 55 and r >= g - 5 and r > b + 10:
                skin.append((x, y))
            # eye / pupil / brow dark
            if lum < 55 and r < 75 and g < 70 and b < 70:
                dark.append((x, y))
    area = float(bw * max(1, band_y1 - band_y0))
    skin_frac = len(skin) / area
    if not skin:
        return {
            "still": str(still), "faceon": False, "reason": "no_face_skin",
            "eye_pairs": 0, "skin_frac": round(skin_frac, 4), "skin_cx_norm": None, "sym": None,
        }
    skin_cx = sum(x for x, _ in skin) / len(skin)
    skin_cx_norm = (skin_cx - x0) / float(bw)  # 0=left edge of teal, 1=right
    # bilateral symmetry of skin about teal center
    mid = (x0 + x1) / 2.0
    left_n = sum(1 for x, _ in skin if x < mid)
    right_n = len(skin) - left_n
    sym = min(left_n, right_n) / float(max(left_n, right_n, 1))
    ...
    if len(dark) >= 6:
        buckets = {}
        for x, y in dark:
            # only count dark near skin ys
            buckets[x // 3] = buckets.get(x // 3, 0) + 1
        peaks = [bx for bx, c in buckets.items() if c >= 3]
        peaks.sort()
        for i in range(len(peaks)):
            for j in range(i + 1, len(peaks)):
                dist = (peaks[j] - peaks[i]) * 3
                if 10 <= dist <= 42:
                    eye_pairs += 1
                    break

    # Profile: skin center offset to one side (typically <0.35 or >0.65), low sym
    # Face-on: skin center near middle (0.35–0.65), higher sym, eye pair present
    centered = 0.34 <= skin_cx_norm <= 0.66
    side_profile = skin_cx_norm <= 0.32 or skin_cx_norm >= 0.68
    wide_stance = (bw / float(w) >= 0.14 and bh / float(h) >= 0.24)

    faceon = False
    reason = "ok_profile_or_back"
    if centered and sym >= 0.45 and eye_pairs >= 1 and skin_frac >= 0.03:
        faceon = True
        reason = "centered_bilateral_face"
    elif centered and sym >= 0.55 and skin_frac >= 0.05 and wide_stance:
        faceon = True
        reason = "centered_wide_front_hero"
    elif side_profile and eye_pairs == 0:
        faceon = False
        reason = "ok_side_profile"
    elif side_profile and sym < 0.4:
        faceon = False
        reason = "ok_asymmetric_profile"
    ...
    return {
        ...
        "skin_cx_norm": round(skin_cx_norm, 3),
        "sym": round(sym, 3),
        "reason": reason,
    }
...
def still_check_garnish(clip: Path, tag: str) -> dict:
    ...
    pose_scores = [explorer_faceon_pose_suspect(p) for p in stills]
    heroes = [s for s in scores if s["face_hero"]]
    faceons = [s for s in pose_scores if s["faceon"]]
    report = {
        "tag": tag,
        "clip": str(clip),
...
def main() -> None:
    # v10 default = I2V from hat-free on-colour start (identity lock).
    ...
    if not _truthy("HOS_FLOW_T2V_ONLY") and not _truthy("HOS_FLOW_I2V_FIRST"):
        ...
        print("  v12 default: I2V Ben-fail locks (glasses/lamp/cards)", flush=True)
    ...
    if bad:
        raise SystemExit(f"STOP: refuse remint via v11: {sorted(bad)}")
    ...
    # Promote preferred start to canonical v10 path for trail
    START_FRAME_PREF.parent.mkdir(parents=True, exist_ok=True)
    if start06.resolve() != START_FRAME_PREF.resolve():
        shutil.copy2(start06, START_FRAME_PREF)
        print(f"  promoted start → {START_FRAME_PREF.name}", flush=True)
    ...
    meta: dict = {
        ...
        "raw": str(RAW),
        "continuity": "v12_clean_light_glasses_lamp_cards",
        "parent_v11_sha": PARENT_V11_SHA,
        "start_frame_06": str(START_FRAME_PREF),
        ...
        "locks": [
            ...
            "dark_teal_coat",
            "toy_scale_garnish",
            "profile_or_back_pose",
            "glasses_always_visible",
            "lamp_clean_no_fire",
            "written_element_cards",
        ],
    ...
    print(f"  start_frames={ {k: v.name for k, v in START_FRAMES.items()} }", flush=True)
    print(
        "  LOCKS armed: GLASSES + LAMP_CLEAN + WRITTEN_CARDS + HAT + COLOUR + POSE",
        flush=True,
    )
        ...
        try:
            for i, plate in enumerate(plates):
                ...
                while submitted < max(1, MAX_CREATES - CREATE_OFFSET) and attempt_n < 8:
                    ...
                    try:
                        start = resolve_start_frame(pid) if use_i2v else None
                        if start is not None and not start.exists():
                            raise SystemExit(f"STOP: missing start frame {start}")
                        if start is not None:
                            print(f"  start-frame I2V: {start.name}", flush=True)
                        else:
                            print("  mode: T2V (HOS_FLOW_T2V_ONLY) — Ben locks in prompt", flush=True)
                    ...
                    if reject_reasons:
                        rej = REJECTED / f"{pid}_v12_try{create_n}_{'+'.join(reject_reasons)}.mp4"
                        if rej.exists():
                            rej.unlink()
    ...
    want = len(plates)
    print(f"OK part 04 Flow mint v10 finished ok={ok} want={want}", flush=True)
    if ok < want:
        sys.exit(2)
