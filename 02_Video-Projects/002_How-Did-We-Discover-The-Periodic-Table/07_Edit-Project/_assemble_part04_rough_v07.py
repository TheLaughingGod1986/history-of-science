...
OUT = PROJ / "09_Final-Export/hos_002_part04_rough_v07.mp4"
...
REMINT_V07_IDS = {"05_columns_families", "06_explorer_leaves_gap", "09_risk_bet", "09b_risk_hold"}
...
def main() -> None:
    ...
    dur = probe(OUT)
    reminted = [pid for pid, src in zip(ordered_ids, sources) if src == "v07_veo"]
    print(
        f"LANDED {OUT}\n"
        f"  path={OUT}\n"
        f"  bytes={bytes_n}\n"
    ...
    meta_path = PROJ / "07_Edit-Project/part04_rough_v07_land_meta.json"
        ...
        json.dumps(
            {
                ...
                "method": "Veo 3.1 Fast T2V — real window night sky, no hard fill",
                ...
                "watch": "WATCH_part04_v07.txt",
                "note": (
                    "Hard-sky-fill scrub; real window night sky+moon ~18–21. "
    ...
    watch = ICLOUD / "WATCH_part04_v07.txt"
    watch.write_text(
        "WATCH THIS FILE ONLY (Part 04 rough v07 — scrub hard navy sky fills (real window)):\n"
        f"  {OUT.name}\n\n"
    ...
    (PROJ / "07_Edit-Project/WATCH_part04_v07.txt").write_text(watch.read_text())
    (ICLOUD / "ZZ_OPEN_PART04_V07_ONLY.txt").write_text(
        "Part 04 current cut = hos_002_part04_rough_v07.mp4\n"
    ...
    (ICLOUD / "PART04_NEXT.txt").write_text(
        "Part 04 rough remint LANDED: hos_002_part04_rough_v07.mp4\n"
        "Watch WATCH_part04_v07.txt\n"
