#!/usr/bin/env python3
"""Part 04 v20 rough — Proof in a Flask.

Edit-first recut. Parent v19 stays. Do not overwrite v19.
KEEP STILL CLEAR v19 luma grade. No T2V this pass.
"""
...
OUT = PROJ / "09_Final-Export/hos_001_part04_rough_v20.mp4"
CLIP_LOCK = {
    "01_question_mark_flask": "01_question_mark_flask_v17.mp4",
    "02_boil_broth": "02_boil_broth_v04.mp4",
    "03_dust_in_the_curve": "03_dust_in_the_curve_v01.mp4",
    "04_still_clear": "04_still_clear_v19.mp4",
    ...
    "08_passengers": "08_passengers_v20.mp4",
    "09_sceptics_watch": "09_sceptics_watch_v20.mp4",
    "10_an_address": "10_an_address_v20.mp4",
...
def main() -> None:
    ...
    for p in plates:
        ...
        if not name:
            raise SystemExit(f"STOP: no v20 lock for {p['id']}")
        cand = RAW / name
        clips.append(cand)
        print(f"  {p['id']:28s} {cand.name}", flush=True)
    still_clear = RAW / "04_still_clear_v19.mp4"
    if not veo.already_done(still_clear, min_bytes=400_000):
        raise SystemExit("STOP: 04_still_clear_v19.mp4 missing")
    if sha256(still_clear) != KEEP04_SHA:
        raise SystemExit("STOP: STILL CLEAR v19 hash mismatch — do not grade again")
    parent = PROJ / "09_Final-Export/hos_001_part04_rough_v19.mp4"
    if not parent.exists():
        raise SystemExit("STOP: parent v19 missing")
