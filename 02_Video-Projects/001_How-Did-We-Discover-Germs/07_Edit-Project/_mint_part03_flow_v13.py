#!/usr/bin/env python3
"""Part 03 Flow Veo 3.1 Fast — 09_they_still_sneer T2V only.

SHIP GATE: later doctor plates = THE SAME two men as KEEP 0:49 07_mocked_v12
(mustache younger, grey 1840s beard older, matching dark frocks, finished
faces, same 1840s wood ward — not a different hospital). No new pair. No wig.
No cheap leftover faces. Living. Fast only.

KEEP 01–08 (07_mocked v12 is the locked VECTOR two-shot) + 10 flask.
09 only: Fast T2V, no still, no Add to Prompt, no Ingredients.
...
META = PROJ / "07_Edit-Project/part03_gen_meta_v13.json"
...
T2V_ID = "09_they_still_sneer"
T2V_DEST = RAW / "09_they_still_sneer_v13.mp4"
KEEP = {
    "01_hands_arrive",
    "02_perfume_windows",
    "03_bedside_hands",
...
KEEP_CLIPS = {
    ...
    "04_autopsy_to_ward": RAW / "04_autopsy_to_ward_v01.mp4",
...
def main() -> None:
    ...
    plates = spec["plates"]
    sneer = next(p for p in plates if p["id"] == T2V_ID)
    if "CLEAN-SHAVEN" in sneer["prompt"] or "two DIFFERENT colleagues" in sneer["prompt"]:
        raise SystemExit(
            "STOP: 09 prompt fails ship gate (clean-shaven / different pair). "
            "Rewrite to match KEEP 0:49 before Create."
        )
    if "SHIP GATE" not in sneer["prompt"] or "SAME 1840s wood" not in sneer["prompt"]:
        raise SystemExit(
            "STOP: 09 prompt missing continuity ship gate / same-ward lock."
        )
    ...
    print(f"LOCKED 07 VECTOR sha256={got7} — will not remint", flush=True)
    print(
        "SHIP GATE: 09 must be THE SAME two men + SAME 1840s wood ward as 0:49. "
        "No new pair. No wig. No cheap leftover faces. Fast only.",
        flush=True,
    )
    ...
    missing = [
        p for p in plates
        if p["id"] not in KEEP
        and not veo.already_done(T2V_DEST, min_bytes=400_000)
    ]
