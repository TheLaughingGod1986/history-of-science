#!/usr/bin/env python3
"""Part 05 v03 rough — pull 07 a_map tail CU. Assembly only. No mint.

07_a_map_v01 trucks into the rejected soap_hands germ-macro after ~3s.
Use only the empty-theatre head. Extend 06 SOAP + 08 Pasteur to cover VO.
...
OUT = PROJ / "09_Final-Export/hos_001_part05_rough_v03.mp4"
...
# VO 45.680s. 07 tail is the leftover CU — pull it.
CLIP_USES = {
    "01_old_theatre": 6.20,
    "02_spray_scrub": 6.20,
    "03_protocol": 6.20,
    "04_explorer_scrubs": 6.20,
    "05_theatre_wins": 6.20,
    "06_soap_hands": 8.00,
    "07_a_map": 2.50,
    "08_last_light": 8.00,
}
...
def main() -> None:
    ...
    for p in plates:
        ...
        if not name:
            raise SystemExit(f"STOP: no v03 lock for {p['id']}")
        cand = RAW / name
        clips.append((p["id"], cand))
        print(f"  {p['id']:28s} {cand.name}", flush=True)
    missing = [str(c) for _, c in clips if not veo.already_done(c, min_bytes=400_000)]
    if missing:
    ...
    uses = []
    t0 = 0.0
    for i, (pid, cand) in enumerate(clips):
        u = CLIP_USES[pid]
        native = probe_dur(cand)
        if u > native + 0.05:
            raise SystemExit(f"STOP: {pid} use {u:.2f} > native {native:.2f}")
        uses.append(u)
        print(f"  WIN {pid:28s} {t0:.2f}-{t0 + u:.2f} use={u:.2f}", flush=True)
        t0 = t0 + u - XFADE
    ...
    inputs: list[str] = []
    parts: list[str] = []
    for i, (_pid, c) in enumerate(clips):
        inputs += ["-i", str(c)]
        parts.append(
