#!/usr/bin/env python3
"""Part 05 UAT — remint 04 and 06 once. 05 v02 KEEP. Fast T2V. If Create dies: STOP."""
...
META = PROJ / "07_Edit-Project/part05_gen_meta_v02b.json"
MODEL = "Veo 3.1 - Fast"
PROFILE = Path(
    os.environ.get(
        "ORBIT_FLOW_PROFILE",
...
PLATES = [
    {
        ...
        "out": "04_explorer_scrubs_v02b.mp4",
        "i2v": True,
        "start": EXPLORER,
        "prompt": (
            "Premium Animistry 3D cartoon, History of Science Part 01 v08 / v21. "
            "IMAGE-TO-VIDEO. 1860s operating theatre is the HERO. ONE boy Explorer: "
            "messy brown hair, round gold glasses, teal overcoat, tan vest, brown "
            "bow tie. He washes at a stone basin then WALKS OUT of frame. Theatre "
            "lamps and wooden table remain. The air is empty — no grey balls, no "
            "white orbs, no bokeh spheres, no floating toys, no particles hanging "
            "in space. Exactly one Explorer. No hats. Silent. Continuous 8 seconds. "
            "No Orbit. No readable text. NOT photoreal. No freeze. No Ken Burns."
        ),
        "t2v_fallback": (
            "Premium Animistry 3D cartoon. 1860s theatre HERO. ONE teal-coat boy "
            "with gold glasses washes at a stone basin then leaves. Empty air, "
            "no floating balls. Silent. Continuous 8 seconds."
        ),
    },
    {
        "id": "06_soap_hands",
        "out": "06_soap_hands_v02b.mp4",
        "i2v": False,
        "prompt": (
            "Premium Animistry 3D cartoon, History of Science Part 01 v08 / v21. "
            "Close-up: two period hands and a white bar of soap under a brass tap. "
            "Stone basin, clear water, ordinary white foam. Bare skin with nothing "
            "on it. Empty air — no glowing dots, no coloured beads, no orbs, no "
            "pink shapes, no blue balls, no orange spots. Steam is faint mist only. "
            "Camera drifts slowly. Continuous 8 seconds. Silent. No other people. "
            "No hats. No modern sink. No Orbit. No Explorer. No readable text. "
...
def motion_mean(mp4: Path) -> float:
    tmp = Path(tempfile.mkdtemp(prefix="hos_p05v02b_m_"))
    try:
        subprocess.run(
            [
                "ffmpeg", "-y", "-i", str(mp4),
...
def main() -> None:
    ...
    for old, tag in (
        (RAW / "04_explorer_scrubs_v02.mp4", "spheres"),
        (RAW / "06_soap_hands_v02.mp4", "orbs"),
    ):
        if old.exists():
            reject = old.with_name(f"_rejected_{tag}_{old.name}")
            if not reject.exists():
                old.rename(reject)
                print(f"  archived {reject.name}", flush=True)
    meta: dict = {"engine": "flow-ui", "model": MODEL, "plates": []}
    profile = flow.profile_path(PROFILE)
    print(f"Flow profile={profile} model={MODEL} remint=04,06 once", flush=True)
    ...
    with sync_playwright() as p:
        ...
        try:
                ...
                while True:
                    try:
                        ...
                        if used_i2v and (
                            ...
                            print(f"  I2V failed — Fast T2V fallback", flush=True)
                            used_i2v = False
                            continue
            ...
            ctx.close()
    print("OK part 05 v02b 04/06 remint finished", flush=True)
