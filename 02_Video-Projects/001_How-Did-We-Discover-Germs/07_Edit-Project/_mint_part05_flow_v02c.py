#!/usr/bin/env python3
"""Part 05 UAT — remint 04 and 06 once. Fast T2V only. If Create dies: STOP."""
...
META = PROJ / "07_Edit-Project/part05_gen_meta_v02c.json"
MODEL = "Veo 3.1 - Fast"
PROFILE = Path(
    os.environ.get(
        "ORBIT_FLOW_PROFILE",
...
PLATES = [
    {
        ...
        "out": "04_explorer_scrubs_v02c.mp4",
        "prompt": (
            "Premium Animistry 3D cartoon, History of Science Part 01 v08. "
            "1860s surgical theatre is the hero: wooden benches, hanging lamps, "
            "a wooden table. One boy Explorer with messy brown hair, round gold "
            "glasses, a teal trenchcoat, tan vest, and brown bow tie. He washes "
            "wet hands at a brass basin, then walks out of frame. Theatre air is "
            "empty. Lamps and wood remain. Silent. Continuous 8 seconds."
        ),
    },
    {
        "id": "06_soap_hands",
        "out": "06_soap_hands_v02c.mp4",
        "prompt": (
            "Premium Animistry 3D cartoon, History of Science Part 01 v08. "
            "Close-up of two period hands washing under a brass tap. Stone basin. "
            "Clear water. White soap foam. Wet skin. Hands rub the soap. Camera "
            "drifts slowly. Silent. Continuous 8 seconds."
        ),
    },
]
...
def motion_mean(mp4: Path) -> float:
    tmp = Path(tempfile.mkdtemp(prefix="hos_p05v02c_m_"))
    try:
        subprocess.run(
            [
                "ffmpeg", "-y", "-i", str(mp4),
...
def main() -> None:
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            ctx.close()
    print("OK part 05 v02c 04/06 remint finished", flush=True)
