#!/usr/bin/env python3
"""Part 04 v21 — 08_passengers Fast T2V only.

Edit-first covered 04 (still_clear_v03) and 10 (an_address_v04).
No unused living Pasteur of the locked shape has hitchhikers in a sunbeam.
KEEP 01–03, 05–07, 09, 11–12. Do not overwrite v20 dests.
...
META = PROJ / "07_Edit-Project/part04_gen_meta_v21.json"
T2V_ID = "08_passengers"
T2V_DEST = RAW / "08_passengers_v21.mp4"
PROMPT = (
    "Premium Animistry 3D cartoon, 1860s wood lab. EXACTLY ONE classic Pasteur flask. "
    "The flask IS a swan-neck: the glass of the bulb itself draws up into ONE long thin "
    "question-mark S-curve that is the only opening. NOT a round Florence flask with a "
    "straight chimney. NOT a tube nested inside a wider neck. NOT a retort with a side-arm. "
    "NO inner coil. NO funnel. Onion-round bulb sitting FLAT on the wooden bench — "
    "NO flared glass foot, NO pedestal. Broth is grey-clear distilled water, not gold. "
    "A warm sunbeam crosses the room. A few sparse faceless hitchhiker rods drift in the "
    "beam OUTSIDE the glass toward the open tip — garnish only, never a swarm, never faces. "
    "Camera already trucks in the first second. Continuous 8s. NO Explorer. NO Orbit. "
    "Silent. No text. Do NOT freeze. Do NOT nest glass. Do NOT grow a chimney."
)
KEEP_SHA = {
    "07_bloom_cloud": (
...
def main() -> None:
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            ctx.close()
    print("OK part 04 v21 T2V mint finished", flush=True)
