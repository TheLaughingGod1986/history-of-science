#!/usr/bin/env python3
"""Part 03 v01 — Bad Air vs Living Seeds.

Chapter card stays: A childbirth ward, 1840s.
Full stack: Flow plates + titles + death-ward bed + living-ward FX.
"""
...
LABEL_DIR = PROJ / "06_Sound-Effects/v08_part03_labels"
FX_DIR = PROJ / "06_Sound-Effects/v08_part03_fx"
MUSIC = PROJ / "05_Music"
BED_RAW = MUSIC / "hos_001_part01_ominous_ward_v12.wav"
BED = MUSIC / "hos_001_part03_ominous_ward_v01_norm.wav"
P01_SFX = PROJ / "06_Sound-Effects/v12"
OUT = PROJ / "09_Final-Export/hos_001_part03_rough_v01.mp4"
ART = Path(
...
def synth_fx() -> dict[str, Path]:
    ...
    # Flask only — plate 10. Hollow glass, not a tray clink.
    ff(
        "-f", "lavfi",
        "-i",
...
def main() -> None:
    ...
    if missing:
        raise SystemExit("missing Flow plates: " + ", ".join(missing))
    if not VO.exists():
    ...
    cards = [("chapter", "A childbirth ward, 1840s", 1.50, 5.00, "right")]
    ...
    # Living ward. No metal. Flask only on plate 10 (~64.5s in).
    ov_parts.append(
        f"[{vo_i}:a]aformat=sample_fmts=fltp:channel_layouts=stereo,"
        f"apad=whole_dur={cut:.6f},atrim=0:{cut:.6f}[vo];"
        f"[{bed_i}:a]atrim=0:{cut:.6f},asetpts=PTS-STARTPTS,volume=0.14[bed];"
        f"[{walla_i}:a]atrim=0:{cut:.6f},volume=0.08[walla_raw];"
        f"[walla_raw][vo]sidechaincompress=threshold=0.025:ratio=10:attack=12:"
        f"release=220:makeup=1.3[walla];"
        f"[{room_i}:a]atrim=0:{cut:.6f},volume=0.05[room];"
        f"[{cough_i}:a]asplit=2[cg1][cg2];"
        f"[cg1]adelay=12800|12800,volume=0.09[c1];"
        f"[cg2]adelay=38500|38500,volume=0.08[c2];"
        ...
        f"[{flask_i}:a]adelay=65500|65500,volume=0.08[flask];"
        f"[vo][bed][walla][room][c1][c2][wood1][wood2][wood3]"
        f"[cloth1][cloth2][cloth3][glass1][glass2][flask]"
        f"amix=inputs=16:duration=first:dropout_transition=0:normalize=0,"
