#!/usr/bin/env python3
"""Part 05 — mint 05 then 08 first. Retry 03 only if both land.

KEEP: 01b, 02c, 04, 06, 07. Do not remint those. Do not remint 01–04 of the film.
If Create dies on 05 or 08: STOP. Do not assemble a partial cut.
"""
...
META = PROJ / "07_Edit-Project/part05_gen_meta_v01d.json"
MODEL = "Veo 3.1 - Fast"
PROFILE = Path(
    os.environ.get(
        "ORBIT_FLOW_PROFILE",
...
def motion_mean(mp4: Path) -> float:
    tmp = Path(tempfile.mkdtemp(prefix="hos_p05d_m_"))
    try:
        subprocess.run(
            [
                "ffmpeg", "-y", "-i", str(mp4),
...
def run_plate(page, plate: dict, meta: dict, *, allow_i2v_fallback: bool) -> None:
    dest = RAW / plate["out"]
    ...
    while True:
        try:
            ...
            if (
                ...
                print(f"  I2V failed ({e}) — falling back to Fast T2V", flush=True)
                used_i2v = False
                continue
        ...
        if still_fail(info):
            ...
            if remint_used:
                META.write_text(json.dumps(meta, indent=2))
                raise SystemExit(
                    f"STOP: still-push on {plate['id']} after one remint. Do not loop."
                )
            remint_used = True
...
def main() -> None:
    ...
    if not T72.exists():
        raise SystemExit(f"STOP: missing Pasteur lock {T72}")
    ...
    print(f"Flow profile={profile} model={MODEL} order=05 then 08 then 03", flush=True)
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            for plate in FIRST:
                run_plate(page, plate, meta, allow_i2v_fallback=True)
            for plate in FIRST:
                dest = RAW / plate["out"]
                if not veo.already_done(dest, min_bytes=400_000):
                    raise SystemExit(
                        f"STOP: {plate['id']} did not land. Do not retry 03."
                    )
            print("\n05 and 08 on disk — one 03 retry", flush=True)
            run_plate(page, THIRD, meta, allow_i2v_fallback=False)
        finally:
            ctx.close()
    print("OK part 05 v01d mint finished", flush=True)
