...
describe("pinned comment", () => {
  it("does not quote a count below the first milestone", () => {
    ...
    const text = buildPinnedCommentText({ question: "Would you have washed your hands?", subscribers: 9 });
    expect(text).not.toMatch(/\d/);
    expect(text.startsWith("Would you have washed your hands?")).toBe(true);
  });
