# Live funnel four — 2026-08-03

## Targets
| YT ID | Short |
|-------|-------|
| `1HuV8o3gOss` | Where Is Everybody? (Fermi) |
| `dPMJQp2gMNc` | Space Is Rude About Distance |
| `rFJoOdQAc9c` | What If Aliens Are Watching Us? |

Full film: https://youtu.be/Mo93x0fxB1Q

## Results

### 1) YouTube — Public + pinned Full film comment
- All three set **Public** (Distance + Zoo were Scheduled; collapsed Schedule panel then Public radio).
- Pinned comment on each:
  ```
...
## Ops notes
- Keep TikTok/Threads Chrome stable with `--disable-extensions --disable-gpu` on `:9222`.
- Meta `:9223` profile `~/.orbit-chrome-meta-dev`.
- Refresh Meta Graph token before API publish path works again.
