...
Instagram `@historyofscience` is **partially** linked to the Orbit Facebook Page, but **not** connected as a Business Suite asset. That is why the Meta Business app shows **Connect Instagram**.

## Current state

| Surface | State |
|---------|--------|
| Facebook Page → Connected Instagram | `@historyofscience` listed, but **Review account connection** (needs IG password) |
...
Portfolio IDs (History of Science):
...
1. Open **Meta Business Suite** app → **History of Science** Page.
2. Tap **Connect Instagram**.
3. Log in as **`@historyofscience`** (Instagram password required).
...
3. Or: Settings → **Instagram accounts** → **Add** → **Claim Instagram Account** → **Log in as historyofscience**.
...
- CDP on `:9222` can open Claim/OAuth and click **Log in as historyofscience**, but the Suite `oidclink` callback does not finish adding the asset (popup closes on a blank callback page).
...
1. Suite → Settings → Instagram accounts shows **`@historyofscience`**
