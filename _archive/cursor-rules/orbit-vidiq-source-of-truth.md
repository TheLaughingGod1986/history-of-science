...
# History of Science YouTube — VidIQ optimization path (mandatory)
...
5. Optional: `vidiq_outliers`, `vidiq_generate_thumbnail` (22 credits) when house thumbs are weak or missing — keep Orbit character / wonder brand; do not ship fearbait comps
...
|---------|--------|-------|
| Title | ≥ **90**, prefer **95–98+** | Keep Orbit voice / `History of Science` on longs. **Reject** fearbait/conspiracy even if score is higher |
...
| Thumbnails | Score ≥ house baseline | Longs: Studio **Thumbnail only** or **Title and thumbnail** A/B/C (3 distinct variants). Shorts: no Studio ABC — optimize cover/title/tags via VidIQ scores |
...
- Related → parent long; pin when long is public
- Prefer one strong cover frame; use `vidiq_generate_thumbnail` only if cover is weak and house assets missing
