...
Score every rough / lock / ship gate against both. Fail either → **UAT FAIL** (stills when picture fails).
...
| Explorer / Part 01 style / microbe locks | **Consistency** scores those house rules on every cut. Do not weaken them. |
| Film-specific lessons (e.g. `PART03_LESSONS.md`) | Extra locks for that minute still apply; these two ALWAYS checks still run on top. |

## Process

- Bake both checks into every Showrunner brief, picture desk QA, and Ben/CoS UAT pass before the next minute.
- Cite this bible + the Cursor rule when failing a cut for consistency or VO-visual miss.
...
- Soft-pass off-model Explorer or scenery-only under specific VO
