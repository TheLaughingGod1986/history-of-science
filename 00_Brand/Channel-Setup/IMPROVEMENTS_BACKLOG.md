...
| # | Job | Why | Done when |
|---|---|---|---|
| 1 | ~~**Land the unmerged film records on `main`.**~~ **Done 25 Sep 2026:** the text records (status, upload results, schedules, plate boards, scripts, social ops) from all 37 tip branches are on main, newest version of each file; main's newer files were kept. Media (mp4/jpg/png, ~361 MB) and `.pid`/`.log` files stay on the branches. **Still to do:** close the branches once Ben has the media he needs from them. | Main stopped at 13 Sep. | ~~Done~~ records; branches still open |
