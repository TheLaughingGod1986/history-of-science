#!/usr/bin/env python3
"""Weekly public audit — run every Monday (ported from Orbit With Ben 25 Sep 2026; see IMPROVEMENTS_BACKLOG.md).
...
Checks
  ...
  - frame 0 of each new Short (i.ytimg.com/vi/<id>/frame0.jpg): a mostly dark frame 0
    (THUMBNAIL_AND_TITLE_RULES.md §3). Orbit's robot detector is not used: it misfires on
    HOS's warm lamp light. Check the Explorer is not at frame 0 by eye.
...
Usage
  python3 weekly_public_audit.py [--date YYYY-MM-DD] [--handle HistoryOfScienceYT]
...
import gate_shorts_open as gate  # noqa: E402  (raw_frame)

ROOT = HERE.parents[2]
WEEKLY = ROOT / "00_Brand/Channel-Setup/audits/weekly"
UA = {"User-Agent": "Mozilla/5.0", "Accept-Language": "en-GB"}
# Frame 0 counts as mostly dark when under 10% of pixels are brighter than grey 60. Set on Orbit's
...
def main() -> int:
    ...
    ap.add_argument("--handle", default="HistoryOfScienceYT")
    ns = ap.parse_args()
    ...
    new_shorts = [s for s in snap["shorts"] if s["id"] not in prev_views] if prev else snap["shorts"]
    for s in new_shorts:
    ...
    # Longs that open with the same four words read as a series label (THUMBNAIL_AND_TITLE_RULES.md §1.3).
    formulas: dict[str, list[dict]] = {}
    for v in snap["longs"]:
        key = " ".join(norm(v["title"] or "").split()[:4])
        if key:
            formulas.setdefault(key, []).append(v)
    formulas = {k: vs for k, vs in formulas.items() if len(vs) > 1}
    subs = snap["subs"] if snap["subs"] is not None else "hidden"
    ...
    if prev:
        ...
        lines += [f"Compared with {prev['captured']}.", "",
                  "| | Now | Change |", "|---|---:|---:|",
                  f"| Subscribers | {subs} | {snap['subs'] - prev['subs']:+d} |" if snap["subs"] is not None and prev.get("subs") is not None else f"| Subscribers | {subs} | — |",
                  f"| Views, newest 48 Shorts | {s_views:,} | {s_views - p_s:+,} |",
                  f"| Views, longs | {l_views:,} | {l_views - p_l:+,} |", ""]
    else:
        lines += ["First snapshot — no comparison yet.", "",
                  f"Subscribers {subs} · newest 48 Shorts {s_views:,} views · longs {l_views:,} views.", ""]

    lines += ["## New since last week", ""]
    new_longs = [v for v in snap["longs"] if v["id"] not in prev_views] if prev else snap["longs"]
    ...
    lines += ["## Flags", ""]
    if not dupes and not flagged and not formulas:
        lines += ["None.", ""]
    for key, vs in formulas.items():
        lines.append(f"- **Same title formula on {len(vs)} longs** (\"{key} …\"): " + " · ".join(f"`{v['id']}` ({v['views']} views)" for v in vs) + " (THUMBNAIL_AND_TITLE_RULES.md §1)")
    for vs in dupes:
    ...
    lines += ["", "## Still needs Studio",
              "", "Public pages cannot show stayed-to-watch, CTR or a silent soundtrack. Add stayed-to-watch at 48 h for each new Short to `audits/SHORTS_LOG.md`, and run `gate_shorts_open.py check` on every export before upload.", ""]
